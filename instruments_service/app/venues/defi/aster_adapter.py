...
class AsterAdapter:
    ...
    def __init__(
        ...
        logger.info(f"✅ AsterAdapter initialized (MVP only: {mvp_only}, base currencies: {len(self.mvp_base_currencies) if self.mvp_base_currencies else 'all'})")
    ...
    def fetch_perpetuals(self, test_data_availability: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.futures_api_base_url}/fapi/v1/exchangeInfo",
                timeout=30
            )
    ...
    def fetch_spot_pairs(self, test_data_availability: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.spot_api_base_url}/api/v3/exchangeInfo",
                timeout=30
            )
            response.raise_for_status()
            exchange_info = response.json()
    ...
    def test_candles_availability(
        self, symbol: str, start_date: datetime, end_date: datetime, interval: str = "1m"
    ) -> bool:
        """
