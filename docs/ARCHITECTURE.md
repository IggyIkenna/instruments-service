...
**Post-2026-05-06 additions** — instruments-service captures per-market lifecycle for predictions (`market_created_at` / `resolution_time` / `settlement_time`) per Plan A Phase 2.A. Sports per-fixture sharding clarified per multi-axis plan: shard atom is `(asset_group=sports, source, data_type, league_id, day)` (NOT per-fixture); fixture_id is row-level in parquet for drill-down. v6 columns wired in canonical_writer for CeFi options/futures (`quote_asset` / `margin_type` / `combo_type` / `leg_weights`).
