...
**Post-2026-05-06 additions** — every instrument record stamps `available_at` per UAC `AVAILABILITY_AT_SEMANTICS`. Predictions add `MarketLifecycle` per market_id (created/resolution/settlement). Sports per-fixture sharding in manifest is at league_id grain, fixture_id is row-level in parquet.
