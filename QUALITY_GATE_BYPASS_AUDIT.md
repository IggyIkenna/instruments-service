...
| **Function/class/method size**  | `engine/orchestrator/sports_reference_core.py`, `engine/orchestrator/sports_reference_fixtures.py` | Both were decomposed out of `orchestrator.py` and REMOVED from this exclude list on 2026-06-11 (post-split, every function passed the 200L/900L gates). Regression found 2026-07-20 (unrelated defi codex-compliance run): `_fetch_teams_and_standings` (205L) and `_write_per_fixture_entities` (253L) regrew past the method gate, `emit_empty_gaps_for_entity` (89L) newly exceeds the class-context method gate. Re-excluded to restore the `CODEX_MAX_VIOLATIONS=3` ceiling. Remediation: decompose the 3 regrown functions (same staged-sibling-module pattern as the 2026-06-11 split); tracked in `issues/instruments_service_codex_compliance_ceiling_drift_2026_07_20.md`. |
