...
```bash
echo "***REDACTED-THEGRAPH-KEY***" | gcloud secrets create graph-api-key \
...
```bash
echo "***REDACTED-THEGRAPH-KEY***" | gcloud secrets versions add graph-api-key \
  --project=central-element-323112 \
  --data-file=-
```
...
6. Secret value: `***REDACTED-THEGRAPH-KEY***`
...
secret_value = '***REDACTED-THEGRAPH-KEY***'
...
```bash
export THE_GRAPH_API_KEY=***REDACTED-THEGRAPH-KEY***
```
