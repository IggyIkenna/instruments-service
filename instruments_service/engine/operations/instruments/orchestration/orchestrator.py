...
class InstrumentsOrchestrator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.cloud_storage: InstrumentStorageProtocol = cast(InstrumentStorageProtocol, StorageAdapter())
    ...
    async def generate_instruments_for_date(
        ...
        root_logger.addHandler(error_warning_counter)  # type: ignore[arg-type]
        ...
        try:
            ...
            if not skip_storage:
                ...
                if success:
                    logger.info(f"✅ Successfully stored {len(instruments_df)} instruments for {date_str}")
                else:
    ...
    async def generate_instruments_date_range(
        ...
        if parallel:
            ...
            async def process_date_with_semaphore(date: datetime) -> dict[str, Any]:
    ...
    def get_processing_stats(self) -> dict[str, str | dict[str, int | None] | None]:
        """Get processing statistics."""
        return {
            "processor": "InstrumentsOrchestrator",
            "config": self.config,
            "batch_processor_config": getattr(self.batch_processor, "config", None),  # type: ignore[reportUnknownMemberType]
        }
    ...
    def cleanup(self) -> None:
        ...
        if hasattr(self.batch_processor, "cleanup"):
            self.batch_processor.cleanup()  # type: ignore[reportUnknownMemberType]
