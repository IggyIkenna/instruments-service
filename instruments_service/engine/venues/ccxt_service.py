...
class CCXTService:
    ...
    def _get_cached_markets(self, cache_key: str) -> dict[str, Any]:
        ...
        if markets_raw is None or not isinstance(markets_raw, dict):
            return {}
        return cast(dict[str, Any], markets_raw)
    ...
    def load_markets(self, venue: str, force_refresh: bool = False) -> CCXTMarketData | None:
        ...
        if not force_refresh and self._is_cache_valid(cache_key):
            logger.debug(
                ...
                f"({len(self._get_cached_markets(cache_key))} markets)"
            )
    ...
    def get_metadata(
        ...
        precision_raw = ccxt_market.get("precision")
        if precision_raw is None or not isinstance(precision_raw, dict):
            precision: dict[str, Any] = {}
        else:
            precision = cast(dict[str, Any], precision_raw)

        limits_raw = ccxt_market.get("limits")
        if limits_raw is None or not isinstance(limits_raw, dict):
            limits: dict[str, Any] = {}
        else:
