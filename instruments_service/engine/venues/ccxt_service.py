...
class CCXTService:
    ...
    def _get_cached_markets(self, cache_key: str) -> dict[str, CCXTMarket]:
        """Get markets dict from cache for logging. Returns empty dict if missing."""
        cached = self._markets_cache.get(cache_key)
        if cached is None:
            return {}
        markets_raw = cached.get("markets")
        if markets_raw is None or not isinstance(markets_raw, dict):
            return {}
        return cast(dict[str, CCXTMarket], markets_raw)
    ...
    def load_markets(self, venue: str, force_refresh: bool = False) -> CCXTMarketData | None:
        ...
        if not force_refresh and self._is_cache_valid(cache_key):
            logger.debug(
                ...
                venue,
                ccxt_exchange_id,
                len(self._get_cached_markets(cache_key)),
            )
        ...
        try:
            ...
            # CCXT has no stubs - cast to dict[str, CCXTMarket] for type safety
            markets = cast(dict[str, CCXTMarket], exchange.load_markets())  # pyright: ignore[reportUnknownMemberType, reportAttributeAccessIssue]
            logger.info(
    ...
    def get_metadata(
        ...
        markets: dict[str, CCXTMarket] | None = ccxt_data.get("markets") if ccxt_data else None
        ...
        ccxt_market: CCXTMarket | None = None
        ...
        # Extract metadata from CCXT market (precision/limits optional per exchange)
        precision: CCXTPrecision | None = ccxt_market.get("precision")
        limits: CCXTLimits | None = ccxt_market.get("limits")
        ...
        if precision:
            tick_size_val = precision.get("price")
            if tick_size_val:
                metadata["tick_size"] = str(tick_size_val)

        # Min size (amount precision)
        if precision:
            min_size_val = precision.get("amount")
            if min_size_val:
    ...
    def get_leverage_limits(
        ...
        try:
            ...
            # CCXT has no stubs - cast to list[CCXTLeverageTier] for type safety
            leverage_tiers: list[CCXTLeverageTier] | None = None
    ...
    def _extract_risk_params_from_tiers(self, leverage_tiers: list[CCXTLeverageTier]) -> CCXTMetadata:
        """
        ...
        for tier in leverage_tiers:
            raw_notional = tier.get("maxNotional")
            if raw_notional is not None and raw_notional > max_notional:
    ...
    def _get_leverage_limits_fallback(self, venue: str) -> CCXTMetadata:
        ...
        venue_defaults = exchange_defaults.get(venue)
        if venue_defaults is None:
            venue_defaults = {}
        elif not isinstance(venue_defaults, dict):
            raise TypeError(f"exchange_defaults[{venue}] must be dict, got {type(venue_defaults).__name__}")
        defaults: CCXTMetadata = cast(CCXTMetadata, venue_defaults)
        if defaults:
            logger.debug(
