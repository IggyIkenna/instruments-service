...
from instruments_service.app.venues.defi.aave.shared import (  # avoid circular import
    AAVE_V3_ETHEREUM_POOL_ADDRESS,
    STATIC_RESERVES,
    STATIC_RISK_PARAMS,
    get_a_token_address,
    get_debt_token_address,
)
...
class AaveV3Adapter(BaseDefiAdapter):
    ...
    # Class-level reference to shared constants (for backward compat; tests assert hasattr)
    STATIC_RISK_PARAMS = STATIC_RISK_PARAMS  # from shared
    ...
    def _get_fallback_reserves(self) -> List[Dict[str, Any]]:
        ...
        """
        static_reserves = STATIC_RESERVES
        logger.warning(
...
""".strip()
    ...
    def _get_a_token_address(self, symbol: str, underlying_address: str) -> str:
        """Delegate to shared get_a_token_address."""
        return get_a_token_address(symbol, underlying_address)

    def _get_debt_token_address(self, symbol: str, underlying_address: str) -> str:
        """Delegate to shared get_debt_token_address."""
        return get_debt_token_address(symbol, underlying_address)
...
""".strip()
    ...
    def _extract_lending_metadata(
        ...
        flash_loan_providers = AAVE_V3_ETHEREUM_POOL_ADDRESS
        ...
        if emode_category_id:
            ...
            if emode_categories and emode_category_id in emode_categories:
                ...
                emode_liquidation_threshold = STATIC_RISK_PARAMS["emode"]["liquidation_thresholds"].get(pair_key)
                emode_liquidation_bonus = STATIC_RISK_PARAMS["emode"]["liquidation_bonus"].get(pair_key)
                logger.debug(
        ...
        if asset_symbol and (not ltv or not liquidation_threshold or not liquidation_bonus or not reserve_factor):
            ...
            if emode_category_id == 1:
                ...
                if not ltv:
                    ltv = STATIC_RISK_PARAMS["emode"]["ltv_limits"].get(pair_key)
                if not liquidation_threshold:
                    liquidation_threshold = STATIC_RISK_PARAMS["emode"]["liquidation_thresholds"].get(pair_key)
                if not liquidation_bonus:
                    liquidation_bonus = STATIC_RISK_PARAMS["emode"]["liquidation_bonus"].get(pair_key)
                if ltv:
                    logger.info(
                ...
                pair_key = f"{asset_symbol}_WETH"
                if not ltv:
                    ltv = STATIC_RISK_PARAMS["standard"]["ltv_limits"].get(pair_key)
                if not liquidation_threshold:
                    liquidation_threshold = STATIC_RISK_PARAMS["standard"]["liquidation_thresholds"].get(pair_key)
                if not liquidation_bonus:
                    liquidation_bonus = STATIC_RISK_PARAMS["standard"]["liquidation_bonus"].get(pair_key)
                if ltv:
                    logger.info(
            ...
            if not reserve_factor:
                reserve_factor = STATIC_RISK_PARAMS["reserve_factors"].get(asset_symbol)
                if reserve_factor:
