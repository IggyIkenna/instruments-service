...
def test_get_instruments_for_date(mock_client):
    ...
    # Mock GCS download
    mock_client.cloud_service.download_from_gcs = Mock(
    ...
    result = mock_client.get_instruments_for_date(date="2023-05-23", venue="TEST")
...
def test_get_instrument_details(mock_client):
    ...
    details = mock_client.get_instrument_details(
        date="2023-05-23", instrument_id="TEST:SPOT_PAIR:BTC-USDT"
    )
        ...
        return_value=pd.DataFrame(
            {
                ...
                "ccxt_symbol": [
                    "BTC/USDT",
                    "ETH/USDT",
                ],  # Add required ccxt_symbol column
