...
Covers: api_football, odds_api, pinnacle, footystats, transfermarkt,
...
class TestPinnacleHelpers:
    ...
    def test_parse_sport_id_valid(self) -> None:
        assert _parse_sport_id("29") == 29
        assert _parse_sport_id("1") == 1

    def test_parse_sport_id_invalid_defaults_to_soccer(self) -> None:
        assert _parse_sport_id("soccer") == 29
        assert _parse_sport_id("") == 29

    def test_extract_leagues_dict_with_league_key(self) -> None:
        raw = {"league": [{"id": 1}, {"id": 2}]}
        result = _extract_leagues(raw)
        assert len(result) == 2

    def test_extract_leagues_dict_with_leagues_key(self) -> None:
        raw = {"leagues": [{"id": 1}]}
        result = _extract_leagues(raw)
        assert len(result) == 1

    def test_extract_leagues_list(self) -> None:
        raw = [{"id": 1}, "not_dict"]
        result = _extract_leagues(raw)
        assert len(result) == 1

    def test_extract_leagues_non_dict_non_list(self) -> None:
        assert _extract_leagues("string") == []

    def test_extract_league_list_dict(self) -> None:
        raw = {"leagues": [{"id": 1, "name": "EPL"}]}
        result = _extract_league_list(raw)
        assert len(result) == 1

    def test_extract_league_list_list(self) -> None:
        raw = [{"id": 1}, "not_dict"]
        result = _extract_league_list(raw)
        assert len(result) == 1


class TestPinnacleAdapterHttp:
    """Async HTTP tests for PinnacleAdapter."""

    @pytest.mark.asyncio
    async def test_get_leagues_parses(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock(
            {
                "leagues": [
                    {"id": 1980, "name": "English Premier League", "homeCountry": "England"},
                    {"id": 2036, "name": "La Liga", "homeCountry": "Spain"},
                ]
            }
        )
        with patch("aiohttp.ClientSession", return_value=mock_session):
    ...
    async def test_get_leagues_skips_non_dict(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock({"leagues": ["not_dict", {"id": 1, "name": "Test"}]})
        with patch("aiohttp.ClientSession", return_value=mock_session):
            leagues = await adapter.get_leagues()
        assert len(leagues) == 1
    ...
    @pytest.mark.asyncio
    async def test_get_odds_no_events(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock({"league": []})
        with patch("aiohttp.ClientSession", return_value=mock_session):
            odds = await adapter.get_odds("29")
        assert odds == []

    @pytest.mark.asyncio
    async def test_fetch_events_parses(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock(
            {
                "league": [
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session):
            events = await adapter._fetch_events(29)
        assert len(events) == 1
        assert events[0].home == "Liverpool"

    @pytest.mark.asyncio
    async def test_fetch_events_error_returns_empty(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock({}, status=500)
        with patch("aiohttp.ClientSession", return_value=mock_session):
            events = await adapter._fetch_events(29)
    ...
    @pytest.mark.asyncio
    async def test_fetch_odds_valid(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock(
            {
                "sportId": 29,
                        ...
                        "events": [
                            {
                                ...
                                "periods": [
                                    {
                                        "number": 0,
                                        "moneyline": {"home": 1.65, "draw": 3.80, "away": 5.50},
                                        "totals": [],
                                        "spreads": [],
                                    }
                                ],
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session):
            odds_resp = await adapter._fetch_odds(29)
        assert odds_resp is not None
        assert len(odds_resp.leagues) == 1

    @pytest.mark.asyncio
    async def test_fetch_odds_non_dict_returns_none(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock([])
        with patch("aiohttp.ClientSession", return_value=mock_session):
            odds_resp = await adapter._fetch_odds(29)
        assert odds_resp is None

    @pytest.mark.asyncio
    async def test_fetch_odds_error_returns_none(self) -> None:
        adapter = PinnacleAdapter(api_key="user:password")
        mock_session = _make_aiohttp_mock({}, status=500)
        with patch("aiohttp.ClientSession", return_value=mock_session):
            odds_resp = await adapter._fetch_odds(29)
    ...
    def test_headers_raises_without_key(self) -> None:
        ...
        with pytest.raises(ValueError, match="Pinnacle adapter requires an API key"):
            adapter._headers()
    ...
    def test_headers_basic_auth(self) -> None:
        ...
        adapter = PinnacleAdapter(api_key="user:password")
        h = adapter._headers()
        expected = base64.b64encode(b"user:password").decode("utf-8")
