...
class CeFiInstrumentProcessor(BaseInstrumentProcessor):
    ...
    def _get_required_list(self, mapping: dict[str, object], key: str, label: str) -> list[str]:
        """Retrieve a required list value from a config dict, raising clearly on misconfiguration."""
        raw = mapping.get(key)
        if raw is None:
    ...
    def _cefi_pre_filter(
        ...
        for symbol_id, symbol_info in instruments_data.items():
            ...
            quote_asset = (parsed.get("quote_asset") or "").upper()
    ...
    def _cefi_mvp_filter(
        self,
        instruments_data: dict[str, dict[str, object]],
        exchange: str,
        canonical_venue: str,
    ) -> dict[str, dict[str, object]]:
        """Apply MVP base-asset allowlist for specific venues."""
    ...
    def _resolve_cefi_available_to(
        self,
        symbol_info: dict[str, object],
        normalized_instrument_type: str,
        enhanced_fields: dict[str, object],
    ) -> str | None:
        """Resolve available_to_datetime from Tardis field or expiry fallback."""
        ...
        if (
            ...
            if expiry_str:
                try:
                    ...
                    day_after = expiry_dt.date() + timedelta(days=1)
                    available_to_datetime = (
                        datetime.combine(day_after, datetime.min.time()).replace(tzinfo=UTC).isoformat()
                    )
    ...
    def _cefi_is_expired(self, available_to_datetime: str | None, target_date: datetime | None) -> bool:
        """Return True if the instrument is expired relative to target_date."""
    ...
    async def _build_cefi_instrument(
        self,
        symbol_id: str,
        symbol_info: dict[str, object],
        exchange: str,
        ...
        """Build an InstrumentDefinition for one symbol; return None to skip."""
        ...
        parsed = self.parse_symbol_components(symbol_id, exchange)
        clean_base = str(parsed.get("base_asset") or "").upper()
        clean_quote = str(parsed.get("quote_asset") or "").upper()
        ...
        enhanced_fields = await populate_derived_fields(
            ...
            inst_type=normalized_instrument_type,
    ...
    async def process_exchange_instruments(
        ...
        instruments_data = self._cefi_pre_filter(instruments_data, exchange, canonical_venue)
        if canonical_venue in self.venue_mapping.spot_mvp_filtered_venues:
            instruments_data = self._cefi_mvp_filter(instruments_data, exchange, canonical_venue)
        ...
        if total_filtered > 0:
            logger.info(
                "Filtering: no_key=%s, same_base_quote=%s, date=%s, expired=%s, expiry=%s, errors=%s, success=%s",
