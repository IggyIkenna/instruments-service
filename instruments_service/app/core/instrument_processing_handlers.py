    ...
    async def process_exchange_instruments(
        ...
        for symbol_id, symbol_info in instruments_data.items():
            ...
            if isinstance(parsed_components, dict):
                base_raw: object = parsed_components.get("base_asset")
                quote_raw: object = parsed_components.get("quote_asset")
                base_asset = (str(base_raw) if base_raw is not None else "").upper()
                quote_asset = (str(quote_raw) if quote_raw is not None else "").upper()
            else:
        ...
        for symbol_id, symbol_info in instruments_data.items():
            try:
                ...
                # Parse components
                parsed_components: dict[str, object] = self.parse_symbol_components(symbol_id, exchange)
                if isinstance(parsed_components, dict):
                ...
                norm_inst_type: str = self.normalize_instrument_type(_type_str) or ""
                ...
                if available_to:
                    try:
                        # available_to is str | None; after truthiness check it is str
                ...
                if available_to_datetime:
                    try:
                        ...
                        logger.debug("⚠️ Could not parse available_to_datetime '%s': %s", available_to_datetime, e)
                ...
                inst_data["instrument_key"] = canonical_key
                inst_data["venue"] = venue_str
                inst_data["instrument_type"] = inst_type_str
                inst_data["symbol"] = symbol
                inst_data["base_asset"] = clean_base
                inst_data["quote_asset"] = clean_quote
                inst_data["settle_asset"] = settle_asset
                inst_data["chain"] = "off-chain"
                inst_data["market_category"] = market_category
                inst_data["exchange_raw_symbol"] = symbol_id
                inst_data["tardis_symbol"] = self._convert_to_tardis_symbol(symbol_id, exchange)
                inst_data["tardis_exchange"] = tardis_exchange
                inst_data["available_from_datetime"] = cast(str, symbol_info.get("availableSince") or "")
                inst_data["available_to_datetime"] = available_to_datetime
                inst_data["data_types"] = data_types_str
                metadata = InstrumentDefinition.model_validate(inst_data)
                ...
                logger.warning("⚠️ Failed to process instrument %s: %s", symbol_id, e)
        # Log statistics
