"""Unit tests — MakerDAO reference-data adapter (sDAI yield-bearing vault-share discovery).
...
Unlike the LST adapters (ankr/stader/stakewise/swell/mantle), sDAI is
``InstrumentType.YIELD_BEARING`` only — it does NOT also resolve under
``InstrumentType.LST`` (no validator staking involved; see maker.py's module
docstring).
"""
...
_EXPECTED_DEPLOY_DATE = datetime(2023, 1, 18, tzinfo=UTC)
...
async def test_get_instruments_filters_on_instrument_type() -> None:
    ...
    # sDAI is not a staking derivative — LST filter must NOT match it (distinguishes
    # this adapter from the LST-only venues it otherwise mirrors).
