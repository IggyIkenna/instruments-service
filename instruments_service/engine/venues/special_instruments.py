...
# Type for instrument definition dictionaries returned by these factory functions
InstrumentDefDict = dict[str, str | float | bool | int | None]


class TradingHoursDict(TypedDict, total=False):
    """Trading hours information for an instrument."""
    ...
    auction_open_utc: str | None
    auction_close_utc: str | None
    early_close_utc: str | None


def create_vix_instrument_definition(target_date: datetime) -> InstrumentDefDict:
    """
...
def get_us_equity_trading_hours(venue: str, instrument_type: str, target_date: datetime) -> TradingHoursDict:
...
def create_bitcoin_etf_instrument_definition(
    ...
    get_exchange_trading_hours: Callable[[str, str, datetime], TradingHoursDict],
) -> InstrumentDefDict | None:
    """
    ...
    if not venue:
        logger.warning("Unknown Bitcoin ETF ticker: %s", ticker)
        return None
