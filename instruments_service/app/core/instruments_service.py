...
class InstrumentsService:
    ...
    async def generate_instruments_for_date(
        ...
        try:
            ...
            if instrument_ids:
                instrument_ids_list: list[str] = (
                    instrument_ids if isinstance(instrument_ids, list) else [str(instrument_ids)]
                )
                ...
                if venues_from_instrument_ids:
                    ...
                    if venues_filter:
                        ...
                        if not venues_filter:
                            logger.warning(
                                f"⚠️ No matching venues between --venues {venues} and "
            ...
            if venues_filter:
                ...
                allowed_cefi_venues: set[str] = set(
                    cast(list[str], self.venue_mapping.all_cefi_venues)
                )  # Canonical venues from CEFI (Tardis + on-chain CLOBs)
                allowed_tradfi_venues: set[str] = set(
                    cast(list[str], self.venue_mapping.all_databento_venues)
                )  # TRADFI venues
                allowed_defi_venues: set[str] = set(cast(list[str], self.venue_mapping.all_defi_venues))  # DEFI venues
            ...
            if cefi:
                ...
                if venues_filter:
                    ...
                    tardis_venue_values: set[str] = set(
                        cast(dict[str, str], self.venue_mapping.tardis_to_venue).values()
                    )
                    ...
                    if cefi_venues:
                        ...
                        # Note: One canonical venue can map to multiple raw exchanges (e.g., OKX -> okex, okex-swap)
                        ...
                        if filtered_exchanges:
                            ...
                            logger.info(
                                f"🔍 Filtered CEFI exchanges by canonical venues {cefi_venues}: {cefi_exchanges}"
                            )
                        else:
                            logger.warning(f"⚠️ No matching CEFI exchanges found for canonical venues: {cefi_venues}")
                            cefi_exchanges = []  # Don't process any CEFI exchanges if no match
                    else:
                    ...
                    if adapter is not None and hasattr(adapter, "base_client"):  # type: ignore[reportAny]
                        ...
                        if hasattr(base_client, "check_venues_access"):  # type: ignore[reportAny]
                            access_results: dict[str, tuple[bool, str]] = cast(
                                ...
                                base_client.check_venues_access(cefi_exchanges),  # type: ignore[reportAny]
                            )
                ...
                if not cefi_exchanges:
                    ...
                    async def process_single_exchange(exchange: str) -> dict[str, InstrumentDefinition]:
                        try:
                            ...
                            instruments_raw = await adapter.fetch_instruments(  # type: ignore[reportAttributeAccessIssue,reportUnknownMemberType]
                                exchange=exchange,
                                target_date=date,
                                force_refresh=force,
                                normalize=True,
                    ...
                    for protocol, _chain in cefi_clob_protocols:
                        try:
                            ...
                            if clob_instruments:
                                ...
                                logger.info(
                                    f"✅ Processed {len(clob_instruments)} instruments from {protocol} "
                                    "(CEFI on-chain CLOB)"
                                )
                        except Exception as e:
            ...
            if tradfi:
                try:
                    ...
                    if tradfi_venues:
                        ...
                        filtered_tradfi_venues: list[str] = [v for v in venues_filter if v in all_databento_exchanges]

                        if filtered_tradfi_venues:
                            databento_exchanges = filtered_tradfi_venues
                            logger.info(
                                f"🔍 Filtered TRADFI exchanges by venues {filtered_tradfi_venues}: "
                                f"{databento_exchanges}"
                            )
                        else:
                        ...
                        async def process_databento_exchange(exchange: str) -> dict[str, InstrumentDefinition]:
                            try:
                                if exchange == "CBOE":
                                    ...
                                    if vix_def_dict:
                                        vix_def = InstrumentDefinition(**vix_def_dict)  # type: ignore[reportAny]
                                    ...
                                    if krwusd_def_dict:
                                        krwusd_def = InstrumentDefinition(**krwusd_def_dict)  # type: ignore[reportAny]
                                    ...
                                    if date >= bitcoin_etf_launch_date:
                                        for ticker in bitcoin_etf_tickers:
                                            ...
                                            if ticker in symbols:
                                                etf_def_dict: dict[str, Any] | None = (
                                                    create_bitcoin_etf_instrument_definition(
                                                        ticker, date, get_us_equity_trading_hours
                                                    )
                                                )
                                                if etf_def_dict:
                                                    etf_def = InstrumentDefinition(**etf_def_dict)  # type: ignore[reportAny]
                                    ...
                                    if non_btc_etf_symbols:
                                        ...
                                        if other_instruments:
                                            ...
                                            logger.info(
                                                f"✅ Processed {len(other_instruments)} additional "
                                                f"instruments from {exchange}"
                                            )
                    ...
                    if venues_filter:
                        ...
                        if defi_venues:
                            ...
                            if defi_protocols:
                                ...
                                logger.info(
                                    f"🔍 Filtered DEFI protocols by venues {defi_venues}: {defi_protocol_names}"
                                )
                            else:
            ...
            if tradfi and venues_filter:
                ...
                if requested_tradfi:
                    ...
                    if "venue" not in instruments_df.columns or instruments_df.empty:
                        ...
                        venues_present = {str(v) for v in unique_venues}  # type: ignore[reportAny]
            ...
            if (
                    ...
                    if row.get("regular_open_utc") and row.get("regular_close_utc"):
                        try:
                            open_dt = cast(
                                pd.Timestamp,
                                pd.to_datetime(
                                    row["regular_open_utc"]  # type: ignore[reportAny]
                                ),
                            )
                            close_dt = cast(
                                pd.Timestamp,
                                pd.to_datetime(
                                    row["regular_close_utc"]  # type: ignore[reportAny]
                                ),
                            )
                ...
                if utc_spanning_instruments:
                    ...
                    open_date_val = cast(
                        date_type,
                        pd.to_datetime(
                            spanning_df.iloc[0]["regular_open_utc"]  # type: ignore[reportAny]
                        ).date(),
                    )
                    logger.info(
