...
class InstrumentDefinition(BaseModel):
    ...
    symbol: str = Field(..., description="Symbol (e.g., BTC-USDT, ETH-USDT@LIN, BTC-USD@INV, BTC-USDT-20250101-50000-CALL)")
    ...
    available_to_datetime: Optional[str] = Field(default=None, description="ISO datetime string when instrument expires (empty for SPOT/PERPETUAL)")
    
    # Data types available for this instrument
    data_types: str = Field(default="trades,book_snapshot_5", description="Comma-separated list of available data types")
    ...
    exchange_raw_symbol: str = Field(default="", description="Raw symbol from exchange API")
    ...
    base_asset_contract_address: Optional[str] = Field(default=None, description="ERC-20 contract address for base asset (DeFi)")
    quote_asset_contract_address: Optional[str] = Field(default=None, description="ERC-20 contract address for quote asset (DeFi)")
    pool_address: Optional[str] = Field(default=None, description="Pool contract address (for DEX pairs, computed from tokens + fee)")
    pool_fee_tier: Optional[int] = Field(default=None, description="Pool fee in basis points (e.g., 500 = 0.05%, 3000 = 0.3%)")
    ...
    def _parse_option_from_symbol_string(self, symbol_string: str) -> tuple[str, str]:
        ...
        try:
            ...
            if symbol_parts[-1] in ['CALL', 'PUT', 'C', 'P']:
                ...
                for i in range(len(symbol_parts) - 2, 1, -1):  # Work backwards, skip option type and stop before base-quote
