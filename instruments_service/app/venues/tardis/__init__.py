        ...
        if not self.api_key:
            if SECRET_MANAGER_AVAILABLE:
                try:
                    ...
                    if self.api_key:
                        logger.info(f"✅ Retrieved Tardis API key from Secret Manager (secret: {secret_name})")
    ...
    def fetch_exchange_instruments(
        ...
        if target_date:
            ...
            if date_filtered_count > 0:
                logger.info(f"📅 Date filter: {len(available_symbols)}/{original_count} instruments available on {date_str}")
        else:
