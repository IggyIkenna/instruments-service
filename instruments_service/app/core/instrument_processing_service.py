...
from instruments_service.app.core.instrument_ops import fetch_databento_instruments as _fetch_databento
from instruments_service.app.core.instrument_ops import fetch_defi_instruments as _fetch_defi
...
class InstrumentProcessingService:
    ...
    def generate_canonical_key(
        ...
        if instrument_type == "SPOT_PAIR":
            ...
            return (
                f"{venue}:OPTION:{base_asset}-{quote_asset}-{expiry_str}-{strike_price}-{option_type}@{option_flavor}"
            )
    ...
    def filter_instruments_by_exchange_config(self, instruments, exchange):
        ...
        canonical_venue = self.normalize_venue(exchange)
        valid_types = self.exchange_config.exchange_instrument_types.get(canonical_venue, ["SPOT_PAIR"])
        valid_quotes = self.exchange_config.valid_quote_currencies.get(canonical_venue, ["USDT"])
            ...
            # UPBIT PARSING (Korean exchange - uses QUOTE-BASE format: KRW-SOL)
                ...
                if "-" in symbol_id:
                    ...
                    if len(parts) == 2:
                        ...
                        logger.debug(f"✅ Upbit QUOTE-BASE format: {symbol_id} → '{base_asset}' + '{quote_asset}'")
            ...
            # COINBASE PARSING (standard BASE-QUOTE format: SOL-USD)
                ...
                if "-" in symbol_id:
                    ...
                    if len(parts) == 2:
                        ...
                        logger.debug(f"✅ Coinbase BASE-QUOTE format: {symbol_id} → '{base_asset}' + '{quote_asset}'")
                        return {"base_asset": base_asset, "quote_asset": quote_asset}
                # Fallback for non-standard symbols
                ...
                if inst_type in ["PERPETUAL", "FUTURE"]:
                    if leverage_limits:
                        ...
                        if leverage_limits.get("initial_margin_rate") is not None:
                            derived_fields["initial_margin_rate"] = leverage_limits["initial_margin_rate"]
    ...
    async def fetch_databento_instruments(
        ...
        Delegates to instrument_ops.databento_fetcher.
        """
        return await _fetch_databento(exchange, symbols, target_date)
    ...
    def fetch_defi_instruments(
        ...
        **kwargs,
        ...
        Delegates to instrument_ops.defi_fetcher.
        """
        return _fetch_defi(self, protocol, chain, target_date, **kwargs)
