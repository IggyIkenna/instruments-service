...
# Lifecycle: permanent
# Delete-when: NA
"""enumerate_expected_universe.py — per-instrument backward-fill (writegate honest-coverage).
...
Per-instrument enumeration is delegated to the ``_enumerate_v2_*`` producers
(dispatched via ``_V2_ENUMERATORS``); each asset_group reads the
``build_instrument_catalogue.py`` catalogue for lifecycle windows and applies
its per-asset_group could-exist rules (venue launch, chain genesis, protocol
launch, source coverage-start, MVP scope, validity matrix, bundle grain).
Requires ``--catalog-path`` (GCS URI or local path).

Example::

    # Scan-only (TradFi)
    python scripts/enumerate_expected_universe.py --asset-group tradfi \\
        --catalog-path gs://<catalogue>.parquet

    # Apply-write (DeFi)
    MANIFEST_PER_VM_SHARDS=true VM_NAME=enum-universe-defi-$(date +%s) \\
    python scripts/enumerate_expected_universe.py \\
        --asset-group defi --catalog-path gs://<catalogue>.parquet \\
...
# ---------------------------------------------------------------------------
# Per-instrument enumerator (Phase 1.A, expected_universe_v2_design)
# ---------------------------------------------------------------------------
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="v2",
        help=(
            "Enumerator version. Only 'v2' is supported (per-instrument-grain "
            "enumerator; the legacy venue-grain v1 dispatch was removed 2026-07-06). "
            "Kept as an explicit flag for launcher-script compatibility."
        ),
    )
    p.add_argument(
        "--catalog-path",
        default=None,
        help=(
            "Path to instruments-service catalog parquet. Required — accepts local "
            "filesystem path or gs:// URI."
        ),
    )
    p.add_argument(
        "--data-types",
        default=None,
        help=(
            "Comma-separated data_type override. Restricts the enumerated data_type "
            "axis to this explicit set — e.g. "
...
def main() -> int:
    ...
    if not catalog_path:
        ...
        return 4
    logger.info("enumerator: loading catalog from %s", catalog_path)
