...
```
gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/
...
```bash
# List GCS structure
gsutil ls gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/

# Check storage size
gsutil du -sh gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/

# Verify metadata
gsutil cat gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/metadata/coverage_report.json
```
...
# Read by_ticker data
df = pd.read_parquet(
    "gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/by_ticker/AAPL/dividends.parquet"
)

# Read by_date data
df = pd.read_parquet(
    "gs://instruments-store-tradfi-{env}-{project_id}/corporate_actions/by_date/day=2024-01-15/dividends.parquet"
)

# Read metadata
import json
client = storage.Client()
bucket = client.bucket("instruments-store-tradfi-{env}-{project_id}")  # Replace {project_id} with actual project ID
