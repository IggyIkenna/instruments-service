...
class UniswapV2Adapter:
    ...
    def _convert_pool_to_instrument(self, pair: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if token0_symbol.upper().startswith('CRV') or token1_symbol.upper().startswith('CRV'):
            logger.debug(f"Skipping pair {pool_id}: Contains Curve LP token ({token0_symbol}/{token1_symbol})")
            return None
        ...
        if (token0_symbol.upper().startswith('BPT') or token1_symbol.upper().startswith('BPT') or
            ...
            logger.debug(f"Skipping pair {pool_id}: Contains Balancer LP token ({token0_symbol}/{token1_symbol})")
            return None
