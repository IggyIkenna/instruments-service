    ...
    def parse_milestones(self) -> List[Dict]:
        ...
        for line in lines[:20]:  # Limit to reasonable number
            ...
            if target_date and target_date != "TBD" and target_date != "N/A":
                try:
                    ...
                    if "Week" in target_date:
                        ...
                        due_date = int(datetime.strptime(target_date, "%Y-%m-%d").timestamp() * 1000)
                except:
                    pass
            ...
            milestones.append({
                ...
                "week": target_date if "Week" in str(target_date) else None  # Store week string
            })
            rows_parsed += 1
...
class ClickUpImporter:
    ...
    def __init__(self, api_token: str, list_id: str, dry_run: bool = False, sprint_start_date: Optional[str] = None, clean_orphaned: bool = False):
        ...
        self.all_tasks_from_status_md = set()  # Track all task names from STATUS.md (for orphan detection)
                ...
                for team in teams.get("teams", []):
                    for member in team.get("members", []):
                        ...
                        if (("ikenna" in username or "igboaka" in username or 
                             ...
                             "ikenna" in full_name or "igboaka" in full_name) and not self.assignee_map.get("Ikenna")):
                            self.assignee_map["Ikenna"] = user_id
                        elif ("harsh" in username or "harsh" in email) and not self.assignee_map.get("Harsh"):
                            self.assignee_map["Harsh"] = user_id
            except Exception as e:
                print(f"⚠️  Could not resolve user IDs from API: {e}")
                if not self.assignee_map.get("Ikenna") or not self.assignee_map.get("Harsh"):
                    print("   Run 'python scripts/get_clickup_user_ids.py' to get user IDs")
                ...
                if field.get("id") == field_id:
                    ...
                    for strategy_name in strategy_names:
                        ...
                        for option in options:
                            ...
                            if strategy_map.get(strategy_name) == option_name or strategy_name in option_name:
                ...
                try:
                    ...
                    if "400" in error_msg:
                        ...
                        if field_data["name"] in self.custom_fields:
                            print(f"✅ Custom field '{field_data['name']}' already exists (ID: {self.custom_fields[field_data['name']]})")
                        else:
                            print(f"⚠️  Could not create custom field '{field_data['name']}': {error_msg}")
                            print(f"   💡 This is usually OK - field might already exist with different format")
                            print(f"   💡 Script will continue and use existing field if found")
                            print(f"   💡 If tasks fail due to missing field, create manually in ClickUp UI")
                    else:
    ...
    def create_task(self, task_data: Dict, parent_id: Optional[str] = None) -> Optional[str]:
        ...
        if "assignees" not in task_data or not task_data.get("assignees"):
            ...
            elif ("Databento" in task_name or "TradFi" in task_name) and "API Access" not in task_name:
        ...
        if self.dry_run:
            ...
            if task_data.get("priority"):
                ...
                details.append(f"priority={priority_map.get(task_data['priority'], task_data['priority'])}")
            if task_data.get("due_date"):
                if isinstance(task_data["due_date"], int):
                    due_date_str = datetime.fromtimestamp(task_data["due_date"] / 1000).strftime("%Y-%m-%d")
            ...
            print(f"🔍 [DRY RUN] Would create task: {indent}{task_data.get('name', 'Unknown')}{parent_info}{detail_str}")
        ...
        try:
            ...
            if existing_task_id:
                ...
                print(f"🔄 Task '{task_name}' already exists (ID: {existing_task_id}), updating...")
            ...
            if len(task_name) > 100:
                ...
                print(f"⚠️  Task name too long ({len(task_name)} chars), truncating to 100 chars")
                ...
                if self.custom_fields.get("Week"):
                    ...
                    if not parent_id:  # Only for top-level tasks (milestones)
                        ...
                        if week_option_id:
                            ...
                            print(f"   ⚠️  Could not find option ID for week '{week_value}', skipping custom field")
            ...
            if "400" in error_msg:
                ...
                if len(task_data.get("name", "")) > 100:
                    print(f"   💡 Task name was truncated from {len(task_data.get('name', ''))} to 100 chars")
    ...
    def import_from_status_md(self, status_md_path: Path):
        ...
        if not self.dry_run:
            ...
            if self.existing_tasks:
                print(f"   Found {len(self.existing_tasks)} existing task(s) - will update instead of duplicate")
        ...
        if len(subtasks_by_parent) == 0:
            ...
            print("   Check STATUS.md formatting - need '## Next Steps' > '### 📋 Planned' section")
        else:
        ...
        for parent_name, parent_data in subtasks_by_parent.items():
            ...
            if milestone_name is None:
                print(f"⚠️  Parent task '{parent_name}' has no milestone, skipping subtasks")
                continue
            
            parent_id = self.task_map.get(milestone_name)
            if not parent_id:
                print(f"⚠️  Parent task '{milestone_name}' (from '{parent_name}') not found, skipping subtasks")
                continue
            ...
            for subtask_data in parent_data["subtasks"]:
                # Convert week string to actual date if needed
                if isinstance(subtask_data.get("due_date"), str) and "Week" in subtask_data.get("due_date", ""):
                    subtask_data["due_date"] = self.get_week_due_date(subtask_data["due_date"])
        ...
        for task_name, depends_on_name in dependency_map.items():
            ...
            if task_id and depends_on_id:
                ...
                if not depends_on_id:
                    print(f"⚠️  Dependency task '{depends_on_name}' not found (will be created as separate task)")
        ...
        if self.clean_orphaned and not self.dry_run:
            ...
            if orphaned_tasks:
                ...
                print(f"   💡 To delete them, run with --clean-orphaned flag (or delete manually in ClickUp)")
        ...
        if not self.dry_run:
            ...
            print(f"   ✅ Due Dates: Automatically set (Week 5-6 = {self.week_dates['Week 5-6'].strftime('%Y-%m-%d')}, Week 7-8 = {self.week_dates['Week 7-8'].strftime('%Y-%m-%d')})")
            print(f"   ✅ Assignees: Automatically assigned (Ikenna for general, Harsh for DeFi/TradFi)")
            print(f"   ✅ Tags: Automatically applied")
            print(f"   ✅ Custom Fields: Automatically created and populated")
        else:
            print(f"   ✅ Dependencies: Will be linked automatically ({len(dependency_map)} dependencies)")
            print(f"   ✅ Priorities: Will be set automatically (DeFi=High, TradFi=Normal)")
            print(f"   ✅ Due Dates: Will be set automatically (Week 5-6 = {self.week_dates['Week 5-6'].strftime('%Y-%m-%d')}, Week 7-8 = {self.week_dates['Week 7-8'].strftime('%Y-%m-%d')})")
            print(f"   ✅ Assignees: Will be assigned automatically (Ikenna for general, Harsh for DeFi/TradFi, Femi for infrastructure/backfill)")
        ...
        print(f"   Week 5-6 ends: {self.week_dates['Week 5-6'].strftime('%Y-%m-%d')} (Dec 19, 2025)")
...
def main():
    ...
    parser.add_argument("--clean-orphaned", action="store_true", help="Delete tasks in ClickUp that are no longer in STATUS.md (use with caution)")
    parser.add_argument("--sprint-start", default="2025-11-07", help="Sprint start date (YYYY-MM-DD). Default: 2025-11-07")
    ...
    if not api_token:
        print("❌ API token not found. Set --api-token or CLICKUP_API_TOKEN env var or add clickup_api_token=... to .env.clickup")
    ...
    importer = ClickUpImporter(api_token, list_id, dry_run=args.dry_run, sprint_start_date=args.sprint_start, clean_orphaned=args.clean_orphaned)
