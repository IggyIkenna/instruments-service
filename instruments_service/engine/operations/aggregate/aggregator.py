...
class InstrumentAggregator:
    ...
    def __init__(self, max_workers: int = 12):
        ...
        logger.info("InstrumentAggregator initialized (max_workers=%s)", max_workers)
    ...
    def aggregate_category(
        ...
        if df.empty:
            logger.warning("No instruments to aggregate for %s", bucket_name)
            return 0
    ...
    def _load_delta_and_merge(
        ...
        list(iterator)
        _prefixes: list[object] = cast(list[object], getattr(iterator, "prefixes", []))
        venue_folders: list[str] = [str(p) for p in _prefixes]
