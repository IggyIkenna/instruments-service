...
class TestInstrumentsQueryHandler:
    ...
    def mock_client(self):
        ...
        client.get_instruments_for_date = Mock(return_value=pd.DataFrame({
            'instrument_key': ['BINANCE-SPOT:SPOT_PAIR:BTC-USDT', 'DERIBIT:OPTION:BTC-USD'],
            'venue': ['BINANCE-SPOT', 'DERIBIT'],
            'instrument_type': ['SPOT_PAIR', 'OPTION'],
            'base_asset': ['BTC', 'BTC'],
            'quote_asset': ['USDT', 'USD']
        }))
        client.get_instruments_date_range = Mock(return_value=pd.DataFrame({
            'instrument_key': ['BINANCE-SPOT:SPOT_PAIR:BTC-USDT'],
            'venue': ['BINANCE-SPOT'],
            'instrument_type': ['SPOT_PAIR']
        }))
        client.get_summary_stats = Mock(return_value={
            'total_instruments': 100,
            'venues': 3,
            'instrument_types': 4
        })
        client.get_instrument_details = Mock(return_value={
            'instrument_key': 'BINANCE-SPOT:SPOT_PAIR:BTC-USDT',
            'venue': 'BINANCE-SPOT'
        })
        client.get_trading_parameters = Mock(return_value={
            'tick_size': '0.01',
            'min_size': '0.001',
            'data_types': ['trades']
        })
        client.get_instruments_by_data_type = Mock(return_value=pd.DataFrame({
            'instrument_key': ['BINANCE-SPOT:SPOT_PAIR:BTC-USDT']
        }))
        client.get_expiring_instruments = Mock(return_value=pd.DataFrame({
    ...
    def handler(self, mock_client):
        ...
        with patch('instruments_service.clients.instruments_client.InstrumentsClient', return_value=mock_client):
            config = {
    ...
    def test_run_details_query(self, handler):
        """Test details query."""
        result = handler.run('2024-01-01', query_type='details', instrument_id='TEST:SPOT_PAIR:BTC-USDT')
        assert result['status'] == 'success'
    ...
    def test_run_trading_params_query(self, handler):
        ...
        result = handler.run('2024-01-01', query_type='trading-params', instrument_id='TEST:SPOT_PAIR:BTC-USDT')
        assert result['status'] == 'success'
    ...
    def test_query_instrument_details_found(self, handler):
        ...
        result = handler._query_instrument_details('2024-01-01', instrument_id='TEST:SPOT_PAIR:BTC-USDT')
        assert result['status'] == 'success'
    ...
    def test_query_trading_parameters_found(self, handler):
        ...
        result = handler._query_trading_parameters('2024-01-01', instrument_id='TEST:SPOT_PAIR:BTC-USDT')
        assert result['status'] == 'success'
    ...
    def test_query_by_data_type(self, handler):
        ...
        result = handler._query_by_data_type('2024-01-01', data_type='trades', venues='BINANCE-SPOT', limit=100)
        assert result['status'] == 'success'
        assert result['data_type'] == 'trades'
        handler.client.get_instruments_by_data_type.assert_called_once()
    ...
    def test_query_by_data_type_venue_list(self, handler):
        ...
        result = handler._query_by_data_type('2024-01-01', data_type='trades', venues=['BINANCE-SPOT'])
        assert result['venue'] == 'BINANCE-SPOT'
    
    def test_query_expiring_instruments(self, handler):
        """Test expiring instruments query."""
        result = handler._query_expiring_instruments('2024-01-01', days_until_expiry=30, instrument_types='OPTION')
        assert result['status'] == 'success'
        assert result['days_until_expiry'] == 30
        handler.client.get_expiring_instruments.assert_called_once()
    ...
    def test_query_expiring_instruments_type_list(self, handler):
        ...
        result = handler._query_expiring_instruments('2024-01-01', instrument_types=['OPTION', 'FUTURE'])
        assert 'results' in result
