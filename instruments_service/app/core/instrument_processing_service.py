...
class InstrumentProcessingConfig:
    ...
    supported_exchanges: List[str] = field(default_factory=lambda: [
        ...
        "bybit", "bybit-spot", "okx", "okx-futures", "okx-swap"
    ])
...
class InstrumentProcessingService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"✅ InstrumentProcessingService initialized: "
    ...
    def generate_canonical_key(
        ...
        if not base_asset or not quote_asset:
            ...
            logger.debug(f"🔄 Skipping instrument with parsing issues: {symbol_id} (exchange: {exchange}) - base:'{base_asset}' quote:'{quote_asset}'")
            return None
        ...
        if instrument_type == 'SPOT_PAIR':
            ...
            if not clean_base or not clean_quote:
                logger.debug(f"Skipping PERPETUAL with invalid/missing base/quote: '{base_asset}'/'{quote_asset}' for {symbol_id}")
                return None
            ...
            if settle_asset == clean_quote:
                ...
                perp_flavor = 'LIN'
                logger.debug(f"⚠️ Could not determine perpetual flavor for {symbol_id}, defaulting to @LIN (settle_asset={settle_asset}, quote={clean_quote}, base={clean_base})")
            ...
            if exchange == 'deribit' and any(strategy in symbol_id for strategy in self.data_config.excluded_deribit_strategies):
    ...
    async def fetch_exchange_instruments(
        ...
        available_symbols_list, date_filtered_count = self.tardis_adapter.fetch_exchange_instruments(
            exchange=exchange,
    ...
    async def process_exchange_instruments(
        ...
        logger.info(f"🔍 Pre-filtering by exchange config: {canonical_venue} accepts types={valid_types}, quotes={valid_quotes}")
        ...
        logger.info(f"🔍 Exchange config filter: {len(pre_filtered)}/{len(instruments_data)} instruments valid for {canonical_venue}")
        ...
        for symbol_id, symbol_info in instruments_data.items():
            try:
                ...
                if not canonical_key:
                    ...
                    if exchange == 'deribit':
                        logger.debug(f"🚫 Deribit: No canonical key for {symbol_id} (type: {symbol_info.get('type', '')})")
                    continue
                ...
                if canonical_key:
                    ...
                    if clean_base == clean_quote and clean_base:
                        ...
                        logger.debug(f"🚫 Filtered nonsensical pair: {symbol_id} ({clean_base}-{clean_quote} - same asset)")
                        continue
                    ...
                    symbol = canonical_key.split(':', 2)[2] if len(canonical_key.split(':')) >= 3 else symbol_id
                    ...
                    enhanced_fields = await self._populate_all_derived_fields(
                        canonical_key, canonical_venue, self.normalize_instrument_type(symbol_info.get('type', '')),
                        clean_base, clean_quote, symbol_id, exchange  # Pass original tardis exchange name
                    )
                    ...
                    mapping_key = (canonical_venue, normalized_instrument_type or 'SPOT_PAIR')
                        ...
                        if expiry_str:
                            try:
                                ...
                                if target_date_only > expiry_date_only:
                                    ...
                                    logger.debug(f"🚫 Filtered: {symbol_id} - expiry {expiry_date_only} < target_date {target_date_only}")
                                    continue
                            except Exception as e:
                    ...
                    metadata = InstrumentDefinition(
                        ...
                        tardis_symbol=self._convert_to_tardis_symbol(symbol_id, exchange),  # ✅ FIXED: Convert to proper Tardis format
        ...
        if total_filtered > 0:
            logger.info(f"🔍 Filtering breakdown for {exchange}: "
    ...
    async def generate_instruments_for_exchanges(
        ...
        logger.info(f"📊 Total instruments processed: {len(all_instruments)} across {len(supported_exchanges)} exchanges")
    ...
    def filter_instruments_by_exchange_config(self, instruments, exchange):
        ...
        valid_types = self.exchange_config.exchange_instrument_types.get(exchange, ['SPOT_PAIR'])
        valid_quotes = self.exchange_config.valid_quote_currencies.get(exchange, ['USDT'])
        ...
        for inst_key, inst_data in instruments.items():
            try:
                ...
                if inst_type not in valid_types:
                    logger.debug(f"❌ Filtered out {inst_key}: {inst_type} not valid for {exchange}")
                    continue
                ...
                if quote_asset not in valid_quotes:
                    logger.debug(f"🚫 Filtered out {inst_key}: quote '{quote_asset}' not in valid quotes {valid_quotes} for {exchange}")
                    continue
    ...
    def _get_ccxt_data_for_venue(self, venue):
        ...
        try:
            ...
            if self._is_ccxt_cache_valid(cache_key):
                logger.debug(f"📋 Using cached CCXT markets for {venue} ({len(self._ccxt_markets_cache[cache_key]['markets'])} markets)")
            ...
            logger.info(f"⚡ Loaded {len(markets)} CCXT markets for {venue} ({ccxt_exchange_id}) - CACHED for reuse")
    ...
    def get_ccxt_metadata_from_cache(self, venue, base_asset, quote_asset, symbol_id):
        ...
        try:
            ...
            if venue == 'BYBIT' and base_asset and quote_asset:
                ...
                possible_symbols.append(f"{base_asset}/{quote_asset}:{quote_asset}")  # BTC/USDT:USDT
                ...
                if 'PERPETUAL' in tardis_symbol:
                    ...
                    if quote_asset == 'USD':
                        possible_symbols.append(f"{base_asset}/USD:{base_asset}")  # BTC/USD:BTC (inverse)
                    elif quote_asset in ['USDC', 'USDT']:
                        possible_symbols.append(f"{base_asset}/{quote_asset}:{quote_asset}")  # BTC/USDC:USDC
                ...
                elif 'FUTURE' in tardis_symbol or any(month in tardis_symbol for month in ['JAN', 'FEB', 'MAR', 'DEC']):
            ...
            ccxt_metadata = {
                ...
                'tick_size': str(ccxt_market.get('precision', {}).get('price', '') or ''),
                # min_size = minimum amount limit (minimum order size)
                'min_size': str(ccxt_market.get('limits', {}).get('amount', {}).get('min', '') or ''),
                # contract_size for derivatives
                'contract_size': str(ccxt_market.get('contractSize', '') or ''),
            }
            
            # Clean None values
            ccxt_metadata = {k: v for k, v in ccxt_metadata.items() if v is not None and v != ''}
            
            logger.debug(f"📊 Enhanced CCXT metadata for {symbol_id}: {len(ccxt_metadata)} fields from cache")
    ...
    def _parse_symbol_components(self, symbol_id, exchange):
        ...
        try:
            ...
            def remove_suffix(s):
                ...
                suffixes = [
                    "USDT", "USDC", "BUSD", "USD", "DAI", "GBP", "TUSD", "EUR", "TRY", "BRL", "JPY", "KRW", "CNY", "HKD",
                    # Regional and unusual fiat quotes (including Mexican Peso)
                    "BRZ", "USDE", "AUD", "RUB", "UAH", "PLN", "RON", "NGN", "ZAR", "IDRT", "VAI", "BIDR", "GEL", "CZK", 
                    "MXN", "ARS", "COP", "CLP", "PEN", "VES", "DAI", "BRL" # Latin American currencies and DAI
                    # Crypto quotes that can be quote assets (from context7)
                    "BTC", "ETH", "BNB", "XRP", "TRX", "ADA", "SOL", "LTC", "DOT", "LINK", "UNI", "AVAX",
                    # Memecoin and additional crypto quotes
                    "DOGE", "SHIB", "PEPE", "FLOKI", "WIF", "BONK", "MEME", "BABYDOGE"
                ]
            ...
            if exchange == 'deribit':
                ...
                # Futures & Options: BTC-25DEC25, BTC-25DEC25-50000-C
                    ...
                    if detected_quote in deribit_valid_quotes and detected_quote in first_part.upper():
                        ...
                        return {'base_asset': first_part, 'quote_asset': 'USD'}  # Deribit default
            ...
            # BYBIT/BINANCE ENHANCED PARSING (from original patterns use context7)  
                    ...
                    for quote in futures_quotes:
                        if base_part.endswith(quote):
                            ...
                            if base and len(base) >= 2:
                                logger.debug(f"✅ Binance futures: {symbol_id} → '{base}' + '{quote}'")
                ...
                crypto_quotes = [
                    'USDT', 'USDC', 'BUSD', 'BTC', 'ETH', 'BNB', 'USD', 'EUR', 'BRZ', 'USDE',
                    # Crypto quotes for compound detection
                    'XRP', 'TRX', 'ADA', 'SOL', 'LTC', 'DOT', 'LINK', 'UNI', 'AVAX', 'ATOM',
                    # Memecoin quotes
                    'DOGE', 'SHIB', 'PEPE', 'FLOKI', 'WIF', 'BONK', 'MEME', 'BABYDOGE',
                    # Fiat and regional currencies (including Mexican Peso)
                    'GBP', 'AUD', 'CAD', 'JPY', 'KRW', 'TRY', 'RUB', 'PLN', 'NGN', 'ZAR',
                    'MXN', 'ARS', 'COP', 'CLP', 'PEN', 'VES', 'CZK', 'RON', 'UAH'
                ]
                    ...
                    if clean_symbol.endswith(quote):
                        ...
                        if base and len(base) >= 2:  # Meaningful base asset (avoid single chars)
                            ...
                            if exchange in ['binance', 'binance-futures']:
                                ...
                                if base[0].isdigit():
                                    logger.debug(f"🚫 Binance invalid: {symbol_id} starts with number")
                                    return {'base_asset': '', 'quote_asset': ''}
                                
                                # Filter USDT as base asset (USDT should be quote, not base)
                                if base == 'USDT':
                                    logger.debug(f"🚫 Binance invalid: {symbol_id} has USDT as base asset")
                                    return {'base_asset': '', 'quote_asset': ''}
                ...
                if base_currency and detected_quote:
                    ...
                    if exchange in ['binance', 'binance-futures']:
                        ...
                        if (base_currency[0].isdigit() and '1000' not in base_currency) or base_currency == 'USDT' or 'USDT' in base_currency:
    ...
    async def _populate_all_derived_fields(self, canonical_key, venue, inst_type, base_asset, quote_asset, symbol_id, exchange=None):
        ...
        try:
            ...
            if inst_type == 'OPTION':
                ...
                logger.debug(f"🔍 Option components for {symbol_id} (exchange: {tardis_exchange}): {option_components}")
                ...
                logger.debug(f"🔧 Derived fields for {canonical_key}: strike='{derived_fields.get('strike')}', option_type='{derived_fields.get('option_type')}')")
            elif inst_type == 'FUTURE':
                # Parse expiry from symbol or use standard
                expiry_date = self._parse_expiry_from_symbol(symbol_id, self.venue_mapping.tardis_to_venue.get(venue, venue).lower())
            ...
            if self.processing_config.enable_ccxt_integration:
                ccxt_data = self.get_ccxt_metadata_from_cache(venue, base_asset, quote_asset, symbol_id)
            ...
            elif venue == 'DERIBIT' and quote_asset in self.exchange_config.valid_quote_currencies.get('DERIBIT', []):
        ...
        try:
            ...
            if exchange == 'deribit':
                ...
                option_strike_patterns = [
                    ...
                    re.compile(r'-(\d{6})-(\d+)-(CALL|PUT)$'),  # Date-Strike-Type: -240329-120000-CALL
    ...
    def _is_problematic_binance_instrument(self, symbol_id: str) -> bool:
        ...
        multiplier_patterns = [
            '1000x', '1000sats', '1000cat', '1000cheems', '1mbabydoge', '1000pepe',
            '1000shib', '1000bonk', '1000floki', '1000rats', '1000btt', '1000lunc',
            '1000000mog', '1000why', '1000000bob', '1000000neiro', '1000wen',
            '1000usual', '1000turbo', '1000xec', '1000000babydoge'
        ]
        ...
        usdt_base_patterns = [
            'usdttry', 'usdtzar', 'usdtuah', 'usdtpln', 'usdtars', 
            'usdtmxn', 'usdtcop', 'usdtclp', 'usdtpen', 'usdtves',
            'usdtngn', 'usdtbrl', 'usdtgel', 'usdtczk', 'usdtron'
        ]
