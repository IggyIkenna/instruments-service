...
class TestRouter:
    def test_route_direct_binance(self) -> None:
    ...
    def test_route_direct_bybit(self) -> None:
        config = ReferenceDataSourceConfig(venue="bybit", data_source="direct")
        adapter = create_reference_data_adapter_for_source(config)
        assert isinstance(adapter, BybitReferenceDataAdapter)

    def test_route_direct_okx(self) -> None:
        config = ReferenceDataSourceConfig(venue="okx", data_source="direct")
        adapter = create_reference_data_adapter_for_source(config)
        assert isinstance(adapter, OKXReferenceDataAdapter)

    def test_route_direct_deribit(self) -> None:
        config = ReferenceDataSourceConfig(venue="deribit", data_source="direct")
        adapter = create_reference_data_adapter_for_source(config)
        assert isinstance(adapter, DeribitReferenceDataAdapter)

    def test_route_direct_coinbase(self) -> None:
        config = ReferenceDataSourceConfig(venue="coinbase", data_source="direct")
        adapter = create_reference_data_adapter_for_source(config)
