...
async def populate_derived_fields(
    ...
    try:
        ...
        _vqc = getattr(service.exchange_config, "valid_quote_currencies", None)
        _raw_deribit = _vqc.get("DERIBIT") if isinstance(_vqc, dict) else None
        deribit_quotes: list[str] = cast(list[str], _raw_deribit) if _raw_deribit is not None else []
