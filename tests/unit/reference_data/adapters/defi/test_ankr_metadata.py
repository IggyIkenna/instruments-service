"""Unit tests — Ankr reference-data adapter (ankrETH LST discovery).

Pure static-registry adapter: ``get_instruments`` returns a hardcoded one-token
catalogue with no network access. Tests are credential-free and offline.
"""

from __future__ import annotations

from datetime import UTC, datetime

import pytest
from unified_api_contracts.internal import InstrumentRecord, InstrumentStatus, InstrumentType

from instruments_service.reference_data.adapters.defi.ankr import (
    AnkrReferenceDataAdapter,
)

_ANKRETH_ADDRESS = "0xE95A203B1a91a908F9B9CE46459d101078c2c3cb"
_EXPECTED_DEPLOY_DATE = datetime(2020, 12, 1, tzinfo=UTC)


def test_venue() -> None:
    assert AnkrReferenceDataAdapter().venue == "ANKR-ETHEREUM"


@pytest.mark.asyncio
async def test_get_instruments_yields_ankreth_record() -> None:
    adapter = AnkrReferenceDataAdapter()
    records = await adapter.get_instruments()
    assert len(records) == 1
    rec = records[0]
    assert isinstance(rec, InstrumentRecord)
    assert rec.venue == "ANKR-ETHEREUM"
    assert rec.instrument_key == "ANKR-ETHEREUM:LST:ANKRETH"
    assert rec.raw_symbol == _ANKRETH_ADDRESS
    assert rec.instrument_type == InstrumentType.LST
    assert rec.base_asset == "ETH"
    assert rec.underlying == "ETH"
    assert rec.status == InstrumentStatus.ACTIVE
    assert rec.available_from_datetime == _EXPECTED_DEPLOY_DATE
    assert rec.base_asset_contract_address == _ANKRETH_ADDRESS
    assert rec.base_asset_decimals == 18


@pytest.mark.asyncio
async def test_get_instruments_filters_on_instrument_type() -> None:
    adapter = AnkrReferenceDataAdapter()
    assert await adapter.get_instruments(instrument_type=InstrumentType.YIELD_BEARING)
    assert await adapter.get_instruments(instrument_type="perpetual") == []


@pytest.mark.asyncio
async def test_get_instrument_lookup() -> None:
    adapter = AnkrReferenceDataAdapter()
    by_addr = await adapter.get_instrument(_ANKRETH_ADDRESS)
    by_symbol = await adapter.get_instrument("ANKRETH")
    assert by_addr is not None and by_symbol is not None
    assert by_addr.instrument_key == by_symbol.instrument_key == "ANKR-ETHEREUM:LST:ANKRETH"
    assert await adapter.get_instrument("NOPE") is None


@pytest.mark.asyncio
async def test_unsupported_market_data_methods_raise() -> None:
    adapter = AnkrReferenceDataAdapter()
    with pytest.raises(NotImplementedError):
        await adapter.get_options_chain("ETH")
    with pytest.raises(NotImplementedError):
        await adapter.get_expiry_calendar("ETH")
    with pytest.raises(NotImplementedError):
        await adapter.get_funding_rate("ANKRETH")
    with pytest.raises(NotImplementedError):
        await adapter.get_ohlcv("ANKRETH")
