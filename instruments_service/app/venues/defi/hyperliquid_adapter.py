...
class HyperliquidAdapter:
    ...
    def __init__(
        ...
        logger.info(f"✅ HyperliquidAdapter initialized (MVP only: {mvp_only}, base currencies: {len(self.mvp_base_currencies) if self.mvp_base_currencies else 'all'})")
    ...
    def _convert_asset_to_instrument(
        ...
        max_leverage_value = max_leverage if isinstance(max_leverage, (int, float)) else (
            max_leverage.get("value", 1) if isinstance(max_leverage, dict) else 1
        )
