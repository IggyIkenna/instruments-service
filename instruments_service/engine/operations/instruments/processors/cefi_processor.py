...
class CeFiInstrumentProcessor(BaseInstrumentProcessor):
            ...
            if not api_key:
                try:
                    ...
                    logger.debug(
                        ...
                        f"(secret: {secret_name}, project: {self._tardis_project_id})"
                    )
    ...
    async def fetch_exchange_instruments(
        ...
        available_symbols_list: list[dict[str, Any]] = []
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"⚠️ Attempt {attempt + 1}/{max_retries} failed for {exchange}: {e}. Retrying in {backoff}s..."
                    )
                    await asyncio.sleep(backoff)
                else:
                    logger.error(f"❌ Failed to fetch {exchange} after {max_retries} attempts: {e}")
                    raise Exception(
    ...
    async def process_exchange_instruments(
        ...
        logger.info(f"🔍 Exchange config filter: {len(pre_filtered)}/{len(instruments_data)} instruments valid")
        ...
        if canonical_venue in self.venue_mapping.spot_mvp_filtered_venues:
            ...
            logger.info(f"🔍 MVP filter: {len(mvp_filtered)}/{len(instruments_data)} instruments")
        ...
        for symbol_id, symbol_info in instruments_data.items():
            try:
                ...
                if not canonical_key:
                    ...
                    if exchange == "deribit":
                        logger.debug(f"🚫 No canonical key for {symbol_id}")
                    continue
                ...
                if clean_base == clean_quote and clean_base:
                    ...
                    logger.debug(f"🚫 Filtered: {symbol_id} ({clean_base}-{clean_quote} - same asset)")
                    continue
                ...
                # but DerivedFieldsServiceProtocol expects object. This is safe - we implement all required methods.
                ...
                if available_to_datetime:
                    try:
                        ...
                        if comparison_date.date() > available_to_dt.date():
                            ...
                            logger.debug(f"🚫 Filtered expired: {symbol_id}")
                            continue
                    except (ValueError, TypeError) as e:
                        logger.debug(f"⚠️ Could not parse available_to '{available_to_datetime}': {e}")
                    ...
                    if expiry_str_filter:
                        try:
                            ...
                            if target_date_only > expiry_date_only:
                                ...
                                logger.debug(f"🚫 Filtered: {symbol_id} - expiry < target_date")
                                continue
                        except (ValueError, TypeError) as e:
        ...
        if total_filtered > 0:
            logger.info(
                f"🔍 Filtering: no_key={filter_stats['no_canonical_key']}, "
                ...
                f"expiry={filter_stats['expiry_filtered']}, "
        ...
        for inst_key, inst_data in instruments.items():
            try:
                ...
                if inst_type not in valid_types:
                    logger.debug(f"❌ Filtered {inst_key}: {inst_type} not valid for {canonical_venue}")
                    continue

                quote_asset: str = (cast(str | None, inst_data.get("quote_asset")) or "").upper()
                if quote_asset not in valid_quotes:
                    logger.debug(f"🚫 Filtered {inst_key}: quote '{quote_asset}' not in {valid_quotes}")
                    continue

                base_asset: str = (cast(str | None, inst_data.get("base_asset")) or "").upper()
                if base_asset in excluded_bases:
                    logger.debug(f"🚫 Filtered {inst_key}: base '{base_asset}' excluded")
                    continue
                ...
                if excluded_patterns:
                    ...
                    for pattern in excluded_patterns:
                        if pattern.upper() in symbol:
                            logger.debug(f"🚫 Filtered {inst_key}: pattern '{pattern}' excluded")
                            excluded_by_pattern = True
                            break
                    if excluded_by_pattern:
                        continue
