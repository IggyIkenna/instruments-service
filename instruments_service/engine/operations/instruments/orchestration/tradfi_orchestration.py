...
class TradFiOrchestrator:
    ...
    def _determine_tradfi_exchanges(
        ...
        if tradfi_venues:
            ...
            if filtered_tradfi_venues:
                logger.info(f"🔍 Processing specified TradFi venues: {filtered_tradfi_venues}")
                return filtered_tradfi_venues
            else:
                logger.warning(f"⚠️ No valid TradFi venues in tradfi_venues={tradfi_venues}, skipping TRADFI processing")
                return []

        elif venues_filter:
            filtered_tradfi_venues = [v for v in venues_filter if v in all_databento_exchanges]
            if filtered_tradfi_venues:
                logger.info(f"🔍 Filtered TRADFI exchanges by venues filter: {filtered_tradfi_venues}")
                return filtered_tradfi_venues
            else:
            ...
            logger.info(f"🔍 No venue filter specified, processing default TRADFI exchanges: {default_exchanges}")
    ...
    async def _process_us_equities(
        ...
        if non_btc_etf_symbols:
            ...
            if other_instruments:
                ...
                logger.info(f"✅ Processed {len(other_instruments)} additional instruments from {exchange}")
    ...
    async def _fetch_databento_instruments(
        ...
        result = await self.processing_service.fetch_databento_instruments(
            exchange=exchange,
            symbols=symbols,
            target_date=date,
        )
