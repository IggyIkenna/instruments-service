...
class ValidationService:
    ...
    def validate_instrument_definition(
        ...
        if not instrument_key.startswith(venue):
            return False, f"Venue mismatch: instrument_key starts with {instrument_key.split(':')[0]}, but venue is {venue}"
        ...
        if len(key_parts) < 2 or key_parts[1] != instrument_type:
            return False, f"Instrument type mismatch: instrument_key has {key_parts[1] if len(key_parts) > 1 else 'unknown'}, but instrument_type is {instrument_type}"
    ...
    def validate_instruments_dataframe(
        ...
        if not duplicates.empty:
            ...
            errors.append(f"Duplicate instrument_keys found: {duplicate_keys[:10]}")  # Limit to first 10
    ...
    def validate_date_range(
        ...
        if start_date > end_date:
            return False, f"Start date {start_date.date()} must be <= end date {end_date.date()}"
