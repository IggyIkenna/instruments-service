...
from unified_internal_contracts import InstrumentStatus

logger = logging.getLogger(__name__)


class InstrumentLifecycleEventType(StrEnum):
    """Instrument lifecycle event types (local definition until unified_internal_contracts exports this)."""
...
class InstrumentLifecycleMonitor:
    ...
    def diff(self, old_keys: set[str], new_records: dict[str, object]) -> list[dict[str, object]]:
        ...
        for key, record in new_records.items():
            expiry_raw = getattr(record, "expiry", None)
            if expiry_raw is None:
                continue
            if not isinstance(expiry_raw, datetime):
                continue
            expiry: datetime = expiry_raw
            if expiry.tzinfo is None:
                expiry = expiry.replace(tzinfo=UTC)

            hours_remaining: float = (expiry - now).total_seconds() / 3600
