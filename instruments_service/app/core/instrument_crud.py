...
class InstrumentCrudMixin:
    ...
    async def generate_instruments_date_range(
        ...
        for date in date_range:
            result, generated, errored = await self._process_single_date(
                date,
                exchanges,
                force,
                cefi,
        ...
        return self._build_date_range_result(date_range, results, total_generated, total_errors)
    ...
    async def _process_single_date(
        ...
        """Process a single date, returning (result, generated_count, error_count)."""
        try:
            ...
            if result.get("status") == "success":
                return result, cast(int, result.get("instruments_generated", 0)), 0
            return result, 0, 1
            ...
            return {"status": "error", "date": date.strftime("%Y-%m-%d"), "error": str(e)}, 0, 1

    @staticmethod
    def _build_date_range_result(
        date_range: list[datetime],
        results: list[dict[str, object]],
        total_generated: int,
        total_errors: int,
    ) -> dict[str, object]:
        """Build the summary result for a date range batch."""
