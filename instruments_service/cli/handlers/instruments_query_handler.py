...
class InstrumentsQueryHandler(ModeHandler):
    ...
    def _query_instruments_list(self, start_date, end_date, **kwargs) -> Dict[str, Any]:
        ...
        if output_format == 'json':
            ...
            result_data = {
                ...
                'sample_instruments': instruments_df.head(10)['instrument_key'].tolist() if not instruments_df.empty else [],
                'venues': instruments_df['venue'].unique().tolist() if not instruments_df.empty else [],
                'instrument_types': instruments_df['instrument_type'].unique().tolist() if not instruments_df.empty else []
            }
    ...
    def _query_by_data_type(self, date, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            'results': {
                'instruments_found': len(instruments_df),
                'instruments': instruments_df['instrument_key'].tolist() if not instruments_df.empty else []
            }
        }
    ...
    def _query_expiring_instruments(self, date, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            'results': {
                ...
                'expiring_instruments': expiring_df[['instrument_key', 'available_to_datetime']].to_dict('records') if not expiring_df.empty else []
            }
        }
