...
# Quote token preference for base/quote ordering
...
def _classify_error(exc: Exception, status: int | None = None) -> str:
    """Map a Uniswap V3 subgraph error to a UAC error code."""
    if status == 429:
        return "RATE_LIMIT"
...
def _order_base_quote(
    sym0: str,
    sym1: str,
) -> tuple[str, str]:
    """Order tokens into (base, quote) using quote-token preference."""
