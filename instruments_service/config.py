...
class VenueMapping:
    ...
    all_tardis_exchanges: List[str] = field(default_factory=lambda: [
    ...
    venue_to_ccxt: Dict[str, str] = field(default_factory=lambda: {
    ...
    tardis_to_venue: Dict[str, str] = field(default_factory=lambda: {
    ...
    venue_instrument_type_to_tardis: Dict[tuple, str] = field(default_factory=lambda: {
        # Binance mappings
    ...
    tardis_exchange_instrument_types: Dict[str, List[str]] = field(default_factory=lambda: {
        'binance': ['SPOT_PAIR'],
...
class DataTypeConfig:
    ...
    instrument_data_types: Dict[str, List[str]] = field(default_factory=lambda: {
    ...
    default_data_types: List[str] = field(default_factory=lambda: [
    ...
    excluded_instrument_types: List[str] = field(default_factory=lambda: [
        'combo'  # Exclude Deribit combo strategies
    ])
    
    # Complex option strategy filters (Deribit specific - exclude complex strategies)
    excluded_deribit_strategies: List[str] = field(default_factory=lambda: [
        'PS-', 'STRG-', 'CBUT-', 'CCOND-', 'PDIAG-', 'PBUT-', 'ICOND-', 'BOX-', 'FS-', 'RR-', 'CSR12-', 'PSR12-', 'CSR13-', 'PSR13-', 'CCAL-', 'CDIAG-'
    ])

@dataclass
class ExchangeInstrumentConfig:
    """Valid instrument types and quote currencies per exchange (CORRECTED canonical venues)"""
    exchange_instrument_types: Dict[str, List[str]] = field(default_factory=lambda: {
    ...
    valid_quote_currencies: Dict[str, List[str]] = field(default_factory=lambda: {
        'BINANCE-SPOT': ['USDT'],               # STRICT: Only USDT (no BNB, ETH, BTC quotes) (fixed: BINANCE -> BINANCE-SPOT)
...
if BASE_SERVICE_CONFIG_AVAILABLE and BaseServiceConfig is not None:
    class InstrumentsServiceConfig(BaseServiceConfig):
        ...
        enable_ccxt_integration: bool = Field(
            default=True,
            description="Enable CCXT metadata enrichment"
        )
        ...
        lookback_days: int = Field(
            default=0,
            description="Lookback days for batch processing"
        )
    ...
    class InstrumentsServiceConfig:
        ...
        def __init__(self, **kwargs):
            ...
            self.gcs_bucket = kwargs.get('gcs_bucket', os.getenv('INSTRUMENTS_GCS_BUCKET', 'instruments-store'))
            self.bigquery_dataset = kwargs.get('bigquery_dataset', os.getenv('INSTRUMENTS_BIGQUERY_DATASET', 'instruments'))
            self.gcp_project_id = kwargs.get('gcp_project_id', os.getenv('GCP_PROJECT_ID', 'central-element-323112'))
            self.bigquery_location = kwargs.get('bigquery_location', os.getenv('BIGQUERY_LOCATION', 'asia-northeast1'))  # Default to asia-northeast1 per .env
