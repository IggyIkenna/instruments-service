...
class InstrumentRefreshScheduler:
    ...
    def start(self, orchestrator: object, publisher: object) -> None:
        ...
        from unified_internal_contracts.instrument_lifecycle import InstrumentLifecycleEvent
        from unified_internal_contracts.pubsub import InternalPubSubTopic

        from .lifecycle_monitor import InstrumentLifecycleMonitor
        ...
        while self._running:
            ...
            try:
                records: dict[str, object] = orchestrator.fetch_all()  # type: ignore[union-attr]
                ...
                for event_dict in events:
                    ...
                    instrument_key = str(event_dict.get("instrument_key", ""))
                    event_dict["venue"] = venue

                    try:
                        lifecycle_event = InstrumentLifecycleEvent(**event_dict)  # type: ignore[arg-type]
                        publisher.publish(  # type: ignore[union-attr]
                            InternalPubSubTopic.INSTRUMENT_LIFECYCLE,
                            lifecycle_event.model_dump(),
                        )
