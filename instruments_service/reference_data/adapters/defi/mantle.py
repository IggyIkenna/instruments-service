...
Discovers the Mantle liquid staking token (mETH) on Ethereum.
...
- mETH contract: see ``_METH_ADDRESS`` below (Etherscan-verified; identical to
  the address MTDS's ``_EVM_LST_STATIC_CONTRACT_ADDRESSES`` uses for the
  on-chain ``mETHToETH()`` rate read).
- Launch date sourced from unified_api_contracts.registry.chain_env.PROTOCOL_LAUNCH_DATES
  (("ETHEREUM", "MANTLE") == 2023-12-04 — Mantle mETH LSP Permissionless Mode
  launch).
"""
...
# Mantle Ethereum mainnet GA (2023-12-04) — mirrors PROTOCOL_LAUNCH_DATES[("ETHEREUM", "MANTLE")].
...
class MantleReferenceDataAdapter(BaseReferenceDataAdapter):
    ...
    Mantle's mETH Liquid Staking Protocol reached Permissionless Mode on
    Ethereum mainnet 2023-12-04. mETH is a yield-bearing liquid staking token
    whose exchange rate vs ETH appreciates as staking rewards accrue.
    """
