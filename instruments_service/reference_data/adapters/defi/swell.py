...
- swETH contract: see ``_SWETH_ADDRESS`` below (Etherscan-verified; identical
  to the address MTDS's ``_EVM_LST_STATIC_CONTRACT_ADDRESSES`` uses for the
  on-chain ``swETHToETHRate()`` rate read).
- Launch date sourced from unified_api_contracts.registry.chain_env.PROTOCOL_LAUNCH_DATES
  (("ETHEREUM", "SWELL") == 2023-04-25 — swETH mainnet "Seawolf unleashed").
"""
...
# Swell Ethereum mainnet GA (2023-04-25) — mirrors PROTOCOL_LAUNCH_DATES[("ETHEREUM", "SWELL")].
...
class SwellReferenceDataAdapter(BaseReferenceDataAdapter):
    ...
    Swell Network's swETH launched on Ethereum mainnet 2023-04-25. swETH is a
