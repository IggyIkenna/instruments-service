"""Solana program pool account discovery utilities.

Discovers ALL accounts owned by a Solana program via the getProgramAccounts
RPC method.  Used for historical pool discovery — the REST APIs only return
currently active pools, but getProgramAccounts returns every account ever
created by the program that still exists on-chain.

Results are cached to GCS (same pattern as the timestamp cache in
``_solana_utils``) so the expensive RPC call (often 10-30s for programs with
thousands of pools) only runs once per program + data_size combination.
"""
