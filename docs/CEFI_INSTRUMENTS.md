...
hours serial** (not measured with concurrency). **The full by_date sweep was NOT run in this pass** (staged rollout:
smoke-test + measure + report, matching the Bybit/Kraken/OKX/Deribit sections above) — see "Known limitations" below.
...
| **`instrument_availability/by_date/` per-day snapshot corpus for BINANCE-FUTURES/BINANCE-DELIVERY not migrated**                                                                                 | 9,234 real files (2019-11-17 to present) still carry the pre-migration `instrument_key` format; `prod/catalog.parquet`'s own fix above will be silently reverted by this venue pair's next catalogue regeneration until these are also fixed. A real, bounded, backed-up 10-file smoke test proved the migration mechanism safe and correct (~2.26s/file real apply throughput; ~5.8h serial ETA for the full 9,234-file sweep, not measured with concurrency) — full sweep is a separate go/no-go decision per the operator's migration-mechanics rule, not executed in this pass. See "BINANCE-FUTURES / BINANCE-DELIVERY `@LIN`/`@INV` migration" above.                                                                                                                                                                                                                  |
