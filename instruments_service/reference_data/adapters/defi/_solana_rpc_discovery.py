"""Solana program-account discovery via getProgramAccounts RPC.

Discovers all accounts owned by a Solana program, used for historical pool
discovery — the REST APIs only return currently active pools, but
getProgramAccounts returns every account ever created by the program that
still exists on-chain.

Results are cached to GCS (same pattern as the timestamp cache in
``_solana_utils.py``) so the expensive RPC call only runs once per
program + data_size combination.
"""
...
# ── Pool discovery cache paths ────────────────────────────────────────
_POOL_DISCOVERY_GCS_BLOB = "_cache/solana_discovered_pools_{protocol}.json"
_POOL_DISCOVERY_LOCAL_DIR: Path = _LOCAL_CACHE_DIR
