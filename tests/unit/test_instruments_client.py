...
class TestInstrumentsClient:
    ...
    def mock_cloud_service(self):
        ...
        service.download_from_gcs = Mock(return_value=pd.DataFrame({
            ...
            'available_from_datetime': ['2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z'],
            'available_to_datetime': ['2024-12-31T00:00:00Z', '2024-12-31T00:00:00Z'],
            'data_types': ['trades,book_snapshot_5', 'options_chain'],
            'symbol': ['BTC-USDT', 'BTC-USD']  # Add symbol column for pattern matching
        }))
        return service
    ...
    def client(self, mock_cloud_service):
        ...
        with patch('instruments_service.clients.instruments_client.StandardizedDomainCloudService', return_value=mock_cloud_service):
    ...
    def test_filter_by_date_availability(self, client):
        ...
        df = pd.DataFrame({
            'available_from_datetime': ['2024-01-01T00:00:00Z', '2024-01-02T00:00:00Z'],
            'available_to_datetime': ['2024-12-31T00:00:00Z', '2024-12-31T00:00:00Z']
        })
