...
def _row_has_parquet(
    ...
    for p in paths:
        try:
            if storage_client.blob_exists(bucket, p):
                return True
        except Exception:
            continue
    return False
...
def _list_day_parquets(storage_client: StorageClient, bucket: str, day: str) -> set[str]:
    ...
    return {b.name for b in storage_client.list_blobs(bucket, prefix=prefix) if b.name.endswith(".parquet")}
...
def _build_existing_set(
    ...
    def _list_day(day: str) -> set[str]:
        try:
            return _list_day_parquets(storage_client, bucket, day)
        except Exception:
            return set()
...
def main(argv: list[str]) -> int:
    ...
    storage_client = get_storage_client(project_id="central-element-323112")

    logger.info("Reading canonical manifest…")
    raw = storage_client.download_bytes(BUCKET, INDEX_BLOB)
    ...
    existing = _build_existing_set(storage_client, BUCKET, distinct_days, args.workers, flat_paths)
    ...
    buf.seek(0)
    storage_client.upload_from_file_obj(BUCKET, INDEX_BLOB, buf, content_type="application/octet-stream")
