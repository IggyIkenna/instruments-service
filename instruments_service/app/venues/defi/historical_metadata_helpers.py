            ...
            if funding_rates and len(funding_rates) > 0:
                ...
                if earliest_time_ms:
                    ...
                    if earliest_dt <= now and days_ago > 30:
                        logger.debug(f"Found earliest funding rate for {symbol}: {earliest_dt.isoformat()}")
                        return earliest_dt
                    else:
                        logger.debug(f"Aster funding rate for {symbol} is too recent ({days_ago} days ago), using default")
                        return None
