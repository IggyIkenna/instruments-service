...
class InstrumentsOrchestrator(OrchestratorBase):
    ...
    async def generate_instruments_for_date(
        ...
        if instrument_ids:
            ...
            if extracted_venues:
                if venues_filter:
                    logger.info(f"🔍 Merging extracted venues {extracted_venues} with filter {venues_filter}")
                    venues_filter = list(set(venues_filter) | set(extracted_venues))
                else:
                    logger.info(f"🔍 Using venues extracted from instrument_ids: {extracted_venues}")
        ...
        logger.info(f"📅 Starting instrument generation for {date_str}")
        ...
        try:
            ...
            logger.info(
                f"✅ Completed instrument generation for {date_str}: "
                f"{len(instruments_df)} instruments, {error_count} errors, {warning_count} warnings"
            )

            return response

        except (ValueError, TypeError, KeyError) as e:
            logger.error(f"❌ Critical error during instrument generation: {e}", exc_info=True)
            root_logger.removeHandler(error_warning_counter)
            return {
                "status": "error",
                "date": date_str,
