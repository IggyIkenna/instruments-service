...
# Lifecycle: oneoff
# Delete-when: after defi + tradfi blank-data_type backfill verified complete (0 blank captured rows on 2026-06-27..2026-07-06 for both asset groups)

"""One-off patch — promote ``data_type=''`` → ``'instruments'`` on IS defi + tradfi captures 2026-06-27+.

ROOT CAUSE (issue ``is_cefi_manifest_blank_data_type_since_2026_06_29_2026_07_06`` P2 item -006):
  The same blank-``data_type`` regression that struck cefi (see the sibling
  ``backfill_cefi_blank_instruments_data_type_2026_07_06.py``) also lands on
  defi + tradfi. Because the writer's non-sports branch at
  ``writers.py:218-250`` is shared across cefi/defi/tradfi (line 219 sets
  ``_cat = "defi" if manifest_chain else VENUE_TO_ASSET_GROUP.get(venue_str, "cefi")``)
  the missing ``data_type`` stamp affected all three asset groups. Item (a) of
  the recommended fix — stamp ``data_type="instruments"`` at
  ``record_captured`` emission time — shipped as ``instruments-service@46ba62b``
  and already covers cefi + defi + tradfi in one branch. This script is item
  (b) for defi + tradfi: the one-off backfill that promotes the historical
  blank rows in ``instruments-store-defi-prd`` +
  ``instruments-store-tradfi-prd`` to the canonical value so any downstream
  consumer using the canonical filter
  (``capture_status=='captured' AND data_type=='instruments'``) stops silently
  missing 10 days of defi + tradfi captures.

Filter (per the plan-of-record, mirrors the cefi script):
    ``date >= 2026-06-27``
    ``AND capture_status == 'captured'``
    ``AND (data_type is null OR data_type == '')``
    ``AND venue != ''``  (venue-grain captures — NOT AG-grain rows)
...
Dry-run by default. ``--apply --confirm`` mutates the live ``_index`` for each
asset group.

Post-run verification: reads the fresh ``_index`` back for each asset group and
reports the count of blank captured rows on ``date >= 2026-06-27``; the exit is
loud-fail if the count is > 0 after ``--apply``.
...
Usage::
    ...
    # inspect what will change on both defi + tradfi (dry-run)
    cd instruments-service
    .venv/bin/python scripts/backfill_defi_tradfi_blank_instruments_data_type_2026_07_06.py

    # mutate the live _index for both buckets
    .venv/bin/python scripts/backfill_defi_tradfi_blank_instruments_data_type_2026_07_06.py --apply --confirm

SSOT: ``plans/active/issues/is_cefi_manifest_blank_data_type_since_2026_06_29_2026_07_06.md``
todo ``[DATA] P2. Verify defi + tradfi are not affected ...``.
"""
...
_ASSET_GROUPS = ("defi", "tradfi")


def _resolve_is_bucket(asset_group: str) -> str:
    return resolve_bucket_name(
        cloud="gcp", kind="instruments-store", asset_group=asset_group, deployment_env="prod"
    )
...
def _identify_blank_rows(df: pd.DataFrame) -> pd.Index:
    ...
    logger.info(
        "Blank-data_type captured rows on %s+: %d of %d total venue-grain captured (%.1f%%)",
        _CUTOFF_DATE,
        len(idx),
...
def _process_asset_group(asset_group: str, apply: bool) -> int:
    """Return 0 on success, non-zero exit code on failure. Skips writes if already clean."""
    bucket = _resolve_is_bucket(asset_group)
    logger.info("=== %s ===", asset_group.upper())
    ...
    if len(blank_idx) == 0:
        logger.info(
            "Already clean — 0 blank %s captured rows on %s+; nothing to do.", asset_group, _CUTOFF_DATE
        )
        return 0
    ...
    if captured_after != captured_before:
        logger.error(
            "SAFETY GATE FAILED (%s): captured row count changed %d → %d after fix — aborting write",
            asset_group,
            captured_before,
            captured_after,
        )
    ...
    logger.info(
        "Safety gate OK (%s): captured row count preserved at %d before and after the fix.",
        asset_group,
        captured_before,
    )
    ...
    if not apply:
        logger.info(
            "DRY-RUN (%s): %d blank captured rows would be promoted to data_type='%s'. "
    ...
    if residual > 0:
        logger.error(
            "Post-run verification FAILED (%s): %d blank captured rows remain on %s+ (expected 0)",
            asset_group,
            residual,
            _CUTOFF_DATE,
        )
    ...
    logger.info(
        "✅ Done (%s) — promoted %d rows to data_type='%s'. Post-run verify: 0 blank captured rows on %s+.",
        asset_group,
        len(blank_idx),
        _REFERENCE_DATA_TYPE,
        _CUTOFF_DATE,
...
def main() -> int:
    ...
    exit_code = 0
    for asset_group in _ASSET_GROUPS:
        rc = _process_asset_group(asset_group, apply)
        if rc != 0:
            exit_code = rc
            # Continue to the next asset group so we surface all failures in one run.
