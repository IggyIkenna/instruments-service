...
from unified_cloud_services import create_instruments_client, setup_cloud_logging

# Setup logging (codex: use logger, not print)
setup_cloud_logging(log_level="INFO", json_format=False)
...
def query_instruments_example():
    ...
    logger.info(f"Retrieved {len(instruments_df)} instruments for {date}")
    if not instruments_df.empty:
        logger.info(
            f"Sample instruments:\n{instruments_df[['instrument_key', 'venue', 'instrument_type', 'symbol']].head()}"
        )
...
def query_instruments_filtered():
    ...
    logger.info(f"Retrieved {len(instruments_df)} BTC-USDT perpetuals")
    if not instruments_df.empty:
        logger.info(f"Instruments:\n{instruments_df[['instrument_key', 'symbol', 'base_asset', 'quote_asset']].head()}")
...
def query_instrument_details():
    ...
    if details:
        logger.info(f"Found instrument: {instrument_id}")
        logger.info("Details:")
...
def query_summary_stats():
    ...
    logger.info("Summary statistics for 2023-05-23:")
...
def query_by_data_type():
    ...
    logger.info(f"Found {len(instruments_df)} instruments with liquidations data")
    if not instruments_df.empty:
        logger.info(f"Sample instruments:\n{instruments_df[['instrument_key', 'symbol', 'data_types']].head()}")
...
def query_date_range():
    ...
    logger.info(f"Retrieved {len(instruments_df)} unique instruments across date range")
    if not instruments_df.empty:
        logger.info(f"Sample instruments:\n{instruments_df[['instrument_key', 'venue', 'symbol']].head()}")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("This demonstrates how to query instruments data using unified-cloud-services")
    logger.info("=" * 60)
