...
class TestCloudDataProvider:
    ...
    def mock_cloud_service(self):
        ...
        service.query_bigquery = Mock(return_value=pd.DataFrame({
            'instrument_key': ['TEST:SPOT_PAIR:BTC-USDT']
        }))
        return service
    ...
    def provider(self, mock_cloud_service, mock_cloud_target):
        ...
        with patch('instruments_service.app.core.cloud_data_provider.create_market_data_service', return_value=mock_cloud_service), \
             patch('instruments_service.app.core.cloud_data_provider.CloudTarget', return_value=mock_cloud_target):
