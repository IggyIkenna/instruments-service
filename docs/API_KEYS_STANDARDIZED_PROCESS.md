...
| `selective_validation` | `validate_required_api_keys(venues)` → get_secret_with_fallback | ✅ |
| `dependency_checker` | get_secret_with_fallback | ✅ |
| `conftest` | get_secret_with_fallback for Tardis | ✅ |
| `scripts/test_batch_cost_comparison.py` | `os.environ.get("DATABENTO_API_KEY")` | ❌ Use get_secret_with_fallback |
| `scripts/find_subgraph_ids.py` | `os.environ.get("THEGRAPH_API_KEY", "test-key")` | ❌ Use get_secret_with_fallback |
...
4. Use `get_secret_with_fallback(secret_name=config.{service}_secret_name, fallback_env_var="{SERVICE}_API_KEY")` in code.
...
- **Scripts:** Use get_secret_with_fallback, not os.environ.get, for consistency.
