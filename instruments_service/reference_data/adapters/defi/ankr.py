...
References:
- https://www.ankr.com/staking/liquid-staking/eth/
- ankrETH contract: see ``_ANKRETH_ADDRESS`` below (Etherscan-verified;
  identical to the address MTDS's ``_EVM_LST_STATIC_CONTRACT_ADDRESSES`` uses
  for the on-chain ``ratio()`` rate read).
- Launch date sourced from unified_api_contracts.registry.chain_env.PROTOCOL_LAUNCH_DATES
  (("ETHEREUM", "ANKR") == 2020-12-01 — Ankr's first LST launch).
"""
...
# Ankr Ethereum mainnet GA (2020-12-01) — mirrors PROTOCOL_LAUNCH_DATES[("ETHEREUM", "ANKR")].
_ANKR_DEPLOY_DATE = datetime(2020, 12, 1, tzinfo=UTC)

# ankrETH token address on Ethereum (18 decimals).
_ANKRETH_ADDRESS = "0xE95A203B1a91a908F9B9CE46459d101078c2c3cb"  # DERIVED from ethereum etherscan
...
class AnkrReferenceDataAdapter(BaseReferenceDataAdapter):
    ...
    Ankr launched its Ethereum liquid staking token 2020-12-01. ankrETH is a
    yield-bearing liquid staking token whose exchange rate vs ETH appreciates
    as staking rewards accrue (on-chain ``ratio()`` returns the inverse ratio,
    shares per bond).
    """
