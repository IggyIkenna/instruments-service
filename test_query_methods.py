...
from unified_cloud_services import create_instruments_client, StandardizedDomainCloudService, CloudTarget
import pandas as pd
...
try:
    ...
    print(f"   Client bucket: {client.cloud_target.gcs_bucket if hasattr(client, 'cloud_target') else 'N/A'}")
...
try:
    ...
    if not instruments_df.empty:
        print(f"   Sample venues: {instruments_df['venue'].unique()[:5].tolist()}")
        print(f"   File size check: {len(instruments_df)} rows, {len(instruments_df.columns)} columns")
except Exception as e:
    print(f"   ❌ Error: {e}")
