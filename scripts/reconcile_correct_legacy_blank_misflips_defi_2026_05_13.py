...
Now Wave 3 adds per-instrument catalog cross-ref to ``_classify_defi`` via
``unified_trading_library.instruments_catalog_reader``. This corrector walks the
~604,951 ``(defi, attempted_failed, error_reason=LegacyBlankErrorReasonError)`` rows,
...
``EXPECTED_INSTRUMENT_DELISTED`` / ``EXPECTED_PRE_VENUE_LAUNCH`` /
``EXPECTED_PRE_GENESIS_CHAIN``, flips:
...
``gs://<defi-bucket>/_index/per_vm/<VM_NAME>.parquet``.
...
* ``unified_trading_library.legacy_reason_classifier._classify_defi`` (extended Wave 3)
...
Example::
    ...
    MANIFEST_PER_VM_SHARDS=true VM_NAME=ikenna-slot2-corrector-defi \\
    python scripts/reconcile_correct_legacy_blank_misflips_defi_2026_05_13.py \\
        --asset-group defi --apply-flips --max-flips 1000000 --confirm

execution:
  owner: ikenna-slot2 (2026-05-14 initial run); one-shot
...
from unified_trading_library.instrument_lifecycle_loader import load_instrument_lifecycle  # noqa: qg-deep-import
...
RECONCILER_NAME = "reconcile_correct_legacy_blank_misflips_defi_2026_05_13"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Wave 3 DeFi corrector: re-classify manifest rows that the 2026-05-07 sweep "
            "conservatively flipped to attempted_failed/LegacyBlankErrorReasonError, "
            "now that the extended _classify_defi has per-instrument catalog cross-ref. "
            "Rows that NOW classify to EXPECTED_INSTRUMENT_NOT_LISTED / "
            "EXPECTED_INSTRUMENT_DELISTED / EXPECTED_PRE_VENUE_LAUNCH / "
            "EXPECTED_PRE_GENESIS_CHAIN are flipped back "
    ...
    parser.add_argument(
        ...
        help="Asset group manifest to reconcile (defi only for this script).",
    )
    parser.add_argument(
        "--bucket",
        default=None,
...
def _download_manifest(bucket_name: str, asset_group: str) -> tuple[pd.DataFrame, str]:
    ...
    with tempfile.NamedTemporaryFile(
        prefix=f"defi-corrector-{asset_group}-",
        suffix=".parquet",
        delete=False,
    ) as tf:
        local_path = tf.name
...
def main() -> int:
    ...
    report_path = report_dir / f"defi-corrector-{asset_group}-{run_ts}.csv"
    ...
    try:
        ...
        if n_corrections == 0:
            logger.info(
                ...
                "Run 'instruments-service build-catalogue --asset-group defi' first."
            )
            _emit_event(
                "RECONCILER_COMPLETED",
                reconciler=RECONCILER_NAME,
        ...
        with tempfile.NamedTemporaryFile(
            prefix=f"defi-corrector-out-{asset_group}-",
            suffix=".parquet",
            delete=False,
        ) as tf:
            out_path = tf.name
