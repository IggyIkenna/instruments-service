...
def delete_all_tasks(client: ClickUpClient, list_id: str, dry_run: bool = False):
    ...
    # Get both active and archived tasks, including all statuses (complete/closed)
    active_tasks = client.get_tasks(
        list_id, archived=False, include_subtasks=True, include_all_statuses=True
    )
    archived_tasks = client.get_tasks(
        list_id, archived=True, include_subtasks=True, include_all_statuses=True
    )
    ...
    print(
        f"   Found {len(active_tasks)} active task(s) and {len(archived_tasks)} archived task(s)"
    )

    # Filter tasks with instruments-service tag
    tasks_to_delete = []
    archived_task_ids = {
        task.get("id") for task in archived_tasks
    }  # Set of archived task IDs
    ...
    tasks_to_delete_sorted = sorted(
        tasks_to_delete, key=lambda x: (not x["is_subtask"], x["name"])
    )
        ...
        try:
            if client.delete_task(task["id"]):
                ...
                if deleted_count % 10 == 0:
                    print(
                        f"   ✅ Deleted {deleted_count}/{len(tasks_to_delete)} tasks..."
                    )
            else:
                failed_count += 1
...
def import_from_tasks_md(
    ...
    print(
        f"   This function is kept for backwards compatibility but will use STATUS.md parser"
    )
...
def main():
    ...
    parser.add_argument(
        "--no-reimport",
        action="store_true",
        help="Don't re-import after cleanup (just delete)",
    )
    parser.add_argument(
        "--source",
