...
_EMPTY_MANIFEST_DF = pd.DataFrame(columns=["date", "data_type", "capture_status"])
...
def test_dep_check_finds_canonical_per_league_pipeline_mode_fixtures() -> None:
    ...
    with (
        patch(
            "instruments_service.reference_data.sports_dependency.read_availability_index",
            return_value=_EMPTY_MANIFEST_DF,
        ),
        patch(
