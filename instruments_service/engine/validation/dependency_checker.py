...
Usage:
    from instruments_service.engine.validation import DependencyChecker
...
import pandas as pd
from unified_cloud_services import CloudTarget, StandardizedDomainCloudService, get_secret_with_fallback
...
class DependencyChecker:
    ...
    def check_external_apis(
        ...
        for category in categories:
            apis_raw = self.EXTERNAL_APIS.get(category)
            if apis_raw is None:
                apis = []  # Category not in config - skip (explicit optional)
            else:
                if not isinstance(apis_raw, list):
                    raise TypeError(f"EXTERNAL_APIS[{category}] must be list, got {type(apis_raw).__name__}")
                apis = apis_raw
