...
def _make_venue_mapping(
    ...
    vm.all_defi_venues = defi or ["UNISWAP_V3", "AAVE_V3"]
    return vm
...
# ---------------------------------------------------------------------------
# From test_processing_handlers_coverage
# ---------------------------------------------------------------------------
...
class TestCloudDataProviderAdditional:
    def _patch_uci(self, ds_return: object, ac_return: object = None):
        ...
        return (
            patch(f"{_MODULE}.get_data_source", return_value=mock_ds),
            patch(f"{_MODULE}.get_analytics_client", return_value=mock_ac),
            mock_ds,
            mock_ac,
        )
    ...
    def test_get_instruments_from_gcs_with_category_delegates(self) -> None:
        ...
        with (
            ...
            patch(f"{_MODULE}.get_data_source"),
            patch(f"{_MODULE}.get_analytics_client"),
        ):
