...
import pandas as pd
from instruments_service.cli.handlers.instrument_handler import InstrumentHandler, parse_date, get_date_range
...
class TestInstrumentHandler:
    ...
    def handler(self, mock_instrument_service, mock_cloud_storage, mock_data_provider):
        ...
        with patch('instruments_service.cli.handlers.instrument_handler.InstrumentProcessingService', return_value=mock_instrument_service), \
             patch('instruments_service.cli.handlers.instrument_handler.CloudInstrumentStorage', return_value=mock_cloud_storage):
    ...
    def test_run_delegates_to_execute(self, handler):
        ...
        with patch.object(handler, '_execute_instrument_generation', return_value={'status': 'success'}) as mock_execute:
    ...
    def test_execute_instrument_generation_skip_existing(self, handler, mock_data_provider):
        ...
        with patch('instruments_service.app.core.cloud_data_provider.CloudDataProvider', return_value=mock_data_provider):
    ...
    def test_execute_instrument_generation_force_mode(self, handler, mock_data_provider):
        ...
        with patch('instruments_service.app.core.cloud_data_provider.CloudDataProvider', return_value=mock_data_provider):
            ...
            with patch.object(handler, '_generate_instruments_for_date', return_value={}):
                result = handler._execute_instrument_generation(
                    test_date.strftime('%Y-%m-%d'),
                    test_date.strftime('%Y-%m-%d'),
                    force=True
        ...
        with patch('instruments_service.app.core.cloud_data_provider.CloudDataProvider'), \
             patch.object(handler, '_generate_instruments_for_date', return_value={
                'TEST:SPOT_PAIR:BTC-USDT': Mock(
                    model_dump=Mock(return_value={'instrument_key': 'TEST:SPOT_PAIR:BTC-USDT'})
                )
            }):
            today = datetime.now(timezone.utc).date()
            test_date = today - timedelta(days=1)
    ...
    def test_execute_instrument_generation_exception_handling(self, handler):
        ...
        with patch('instruments_service.app.core.cloud_data_provider.CloudDataProvider'), \
             patch.object(handler, '_generate_instruments_for_date', side_effect=Exception("Test error")):
            today = datetime.now(timezone.utc).date()
            test_date = today - timedelta(days=1)
    ...
    def test_generate_instruments_for_date(self, handler, mock_instrument_service):
        ...
        result = handler._generate_instruments_for_date(today, force=True, exchanges=['binance'])
    ...
    def test_generate_instruments_for_date_all_exchanges(self, handler, mock_instrument_service):
        ...
        today = datetime.now(timezone.utc)
        result = handler._generate_instruments_for_date(today, force=True, exchanges=None)
    ...
    def test_generate_instruments_for_date_exchange_error(self, handler, mock_instrument_service):
        ...
        today = datetime.now(timezone.utc)
        result = handler._generate_instruments_for_date(today, force=True, exchanges=['binance', 'deribit'])
