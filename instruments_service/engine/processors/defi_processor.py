    ...
    **kwargs: DefiProtocolKwargs,
    ...
    try:
        ...
        if protocol.lower() == "uniswap_v3":
            ...
            raw_instruments = cast(dict[str, object], adapter.fetch_markets(target_date=target_date))
        elif protocol.lower() == "etherfi":
            adapter = EtherFiAdapter(chain=chain)
            raw_instruments = cast(dict[str, object], adapter.fetch_lst_instruments())
        elif protocol.lower() == "lido":
            adapter = LidoAdapter(chain=chain)
            raw_instruments = cast(dict[str, object], adapter.fetch_lst_instruments())
        elif protocol.lower() == "morpho":
            adapter = MorphoAdapter(chain=chain)
            raw_instruments = cast(dict[str, object], adapter.fetch_markets())
            ...
            raw_instruments = cast(dict[str, object], adapter.fetch_yield_bearing_instruments())
        ...
        for inst_data in raw_instruments.values():
            if protocol.lower() == "hyperliquid" and isinstance(inst_data, dict):
                inst_d: dict[str, object] = cast(dict[str, object], inst_data)
        ...
        for venue in venues_to_enrich:
            logger.info("⚡ Pre-loading CCXT markets for %s to ensure enrichment works", venue)
            ccxt_data = service.ccxt_service.load_markets(venue)
            if ccxt_data and ccxt_data.get("markets"):
                logger.info(
                    "✅ CCXT markets loaded for %s: %s markets",
                    venue,
                    len(cast(dict[str, object], ccxt_data["markets"])),
                )
            else:
                logger.warning("⚠️ Failed to load CCXT markets for %s", venue)
        ...
        for inst_key, inst_data in raw_instruments.items():
            try:
                ...
                if (
                    ...
                    logger.debug("Skipping %s: base currency '%s' not in MVP list", inst_key, base_asset)
                    continue
                ...
                if quote_asset and quote_asset not in mvp_quotes:
                    ...
                    if quote_asset not in quote_versions or quote_versions[quote_asset] not in mvp_quotes:
                        logger.debug("Skipping %s: quote currency '%s' not in MVP list", inst_key, quote_asset)
                        continue
                ...
                if protocol.lower() == "hyperliquid":
                    ...
                    if ccxt_metadata:
                        ...
                        if manual_metadata:
                            ...
                            logger.debug(
                                "✅ Used manual fallback for %s: tick_size=%s, min_size=%s",
                                inst_def.instrument_key,
                                inst_def.tick_size,
                                inst_def.min_size,
                            )
                        else:
                            logger.debug(
                                "⚠️ CCXT enrichment failed for %s: venue=%s, base=%s, quote=%s",
                                inst_def.instrument_key,
                                venue,
                                inst_def.base_asset,
                                inst_def.quote_asset,
                            )
