...
Discovers the StakeWise V3 liquid staking token (osETH) on Ethereum.

References:
- https://stakewise.io/
- osETH contract: see ``_OSETH_ADDRESS`` below (Etherscan-verified; same address
  MTDS's ``lst_rates_handler.py`` / ``_instruments_metadata.py`` use). Note the
  rate is read from the OsTokenVaultController, not the osETH token itself
  (the ERC-20 has no rate function) — an MTDS-capture detail, irrelevant to
  reference-data instrument discovery here.
- Launch date sourced from unified_api_contracts.registry.chain_env.PROTOCOL_LAUNCH_DATES
  (("ETHEREUM", "STAKEWISE") == 2023-11-28, the StakeWise V3 + osETH launch —
  V2's earlier 2021-03-24 launch predates osETH and is not this token's date).
"""
...
# StakeWise V3 + osETH launch (2023-11-28) — mirrors PROTOCOL_LAUNCH_DATES[("ETHEREUM", "STAKEWISE")].
...
class StakewiseReferenceDataAdapter(BaseReferenceDataAdapter):
    ...
    StakeWise V3 launched its osETH liquid staking token on Ethereum
    2023-11-28. osETH is a yield-bearing token whose exchange rate
    (``convertToAssets()`` on the vault controller) vs ETH appreciates as
