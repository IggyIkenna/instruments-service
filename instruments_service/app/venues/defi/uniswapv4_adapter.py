...
class UniswapV4Adapter:
    ...
    def __init__(
        ...
        if subgraph_url is None:
            ...
            subgraph_url = subgraph_service.get_subgraph_url("uniswap_v4", self.chain, api_key=api_key)
    ...
    def _convert_pool_to_instrument(self, pool: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if token0_symbol.upper().startswith('CRV') or token1_symbol.upper().startswith('CRV'):
            logger.debug(f"Skipping pool {pool_id}: Contains Curve LP token ({token0_symbol}/{token1_symbol})")
            return None
        
        # CRITICAL: Skip Balancer LP tokens (they start with "BPT" or "bb-" prefix)
        if (token0_symbol.upper().startswith('BPT') or token1_symbol.upper().startswith('BPT') or
            token0_symbol.upper().startswith('BB-') or token1_symbol.upper().startswith('BB-')):
            logger.debug(f"Skipping pool {pool_id}: Contains Balancer LP token ({token0_symbol}/{token1_symbol})")
            return None
    ...
    def fetch_spot_pairs(
        ...
        **kwargs
        ...
        Returns:
            Dictionary mapping instrument_key to instrument definition
        """
        pools = self.fetch_pools(base_currency=base_currency, quote_currency_list=quote_currency_list)
